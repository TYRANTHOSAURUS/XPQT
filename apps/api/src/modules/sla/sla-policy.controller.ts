...
export function validateEscalationThresholds(input: unknown): EscalationThreshold[] {
  ...
  if (!Array.isArray(input)) throw AppErrors.validationFailed('sla.threshold_invalid', { detail: 'escalation_thresholds must be an array' });
  const seen = new Set<string>();
  const out: EscalationThreshold[] = [];
  for (const [i, raw] of input.entries()) {
    if (!raw || typeof raw !== 'object') {
      throw AppErrors.validationFailed('sla.threshold_invalid', { detail: `escalation_thresholds[${i}] must be an object` });
    }
    const t = raw as Record<string, unknown>;
    const at = t.at_percent;
    if (typeof at !== 'number' || !Number.isInteger(at) || at < 1 || at > 200) {
      throw AppErrors.validationFailed('sla.threshold_invalid', { detail: `escalation_thresholds[${i}].at_percent must be an integer in [1, 200]` });
    }
    if (!TIMER_SCOPES.includes(t.timer_type as ThresholdTimerScope)) {
      throw AppErrors.validationFailed('sla.threshold_invalid', { detail: `escalation_thresholds[${i}].timer_type must be one of ${TIMER_SCOPES.join(', ')}` });
    }
    if (!ACTIONS.includes(t.action as ThresholdAction)) {
      throw AppErrors.validationFailed('sla.threshold_invalid', { detail: `escalation_thresholds[${i}].action must be one of ${ACTIONS.join(', ')}` });
    }
    if (!TARGET_TYPES.includes(t.target_type as ThresholdTargetType)) {
      throw AppErrors.validationFailed('sla.threshold_invalid', { detail: `escalation_thresholds[${i}].target_type must be one of ${TARGET_TYPES.join(', ')}` });
    }
    ...
    if (targetType === 'manager_of_requester') {
      if (t.target_id !== null && t.target_id !== undefined) {
        throw AppErrors.validationFailed('sla.threshold_invalid', { detail: `escalation_thresholds[${i}].target_id must be null for manager_of_requester` });
      }
    } else {
      if (typeof t.target_id !== 'string' || !UUID_RE.test(t.target_id)) {
        throw AppErrors.validationFailed('sla.threshold_invalid', { detail: `escalation_thresholds[${i}].target_id must be a uuid` });
      }
      targetId = t.target_id;
    }
    const key = `${at}|${t.timer_type}`;
    if (seen.has(key)) {
      throw AppErrors.validationFailed('sla.threshold_invalid', { detail: `escalation_thresholds has duplicate (at_percent=${at}, timer_type=${String(t.timer_type)})` });
    }
    seen.add(key);
    out.push({
      at_percent: at,
