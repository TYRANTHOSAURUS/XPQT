...
describe('ResolverService', () => {
  it('returns unassigned when no context and no fallbacks', async () => {
    const svc = new ResolverService(stubRepo() as never, { resolve: jest.fn().mockResolvedValue(null), resolveForLocation: jest.fn().mockResolvedValue(null), deriveEffectiveLocation: jest.fn().mockResolvedValue(null) } as never);
