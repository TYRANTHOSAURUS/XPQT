...
describe('ApprovalService — booking grant via grant_booking_approval RPC (B.0.D.3)', () => {
  ...
  it('does NOT call grant_booking_approval for non-booking targets', async () => {
    // Ticket-target approvals route through grant_ticket_approval (Step10);
    // visitor_invite stays on the legacy TS path. Verify the dispatcher
    // does NOT invoke the booking-target RPC for either non-booking case.
    const { svc, rpcCalls } = makeBookingService({
      approval: approvalRow({ target_entity_type: 'visitor_invite' }),
...
/**
 * B.2.A.Step10 — spec §3.5. Verifies the ticket-target dispatcher routes
 * through `grant_ticket_approval` (00343) and surfaces the RPC's
 * structured outcomes correctly. Mirrors the booking-grant block above.
 */
describe('ApprovalService — ticket grant via grant_ticket_approval RPC (B.2.A.Step10)', () => {
  afterEach(() => jest.restoreAllMocks());
  ...
  function makeTicketService(opts: {
    ...
    const svc = new ApprovalService(
      supabase as never,
      // TicketService stub — no longer called post-cutover but the
      // constructor still requires it for DI graph compatibility.
      { onApprovalDecision: jest.fn() } as never,
      { onApprovalDecided: jest.fn() } as never,
      { onApprovalDecided: jest.fn() } as never,
      { onApprovalDecided: jest.fn() } as never,
    ...
    // Make `callerCanRespond` always succeed for the test's actor.
    jest
      .spyOn(svc as unknown as { callerCanRespond: () => Promise<boolean> }, 'callerCanRespond')
      .mockResolvedValue(true);
  ...
  it('surfaces the partial_approved RPC outcome to the caller (peers still pending)', async () => {
    const { svc } = makeTicketService({
      approval: ticketApprovalRow({ approval_chain_id: 'chain-1' }),
      rpcStub: () =>
        Promise.resolve({
          data: {
            kind: 'partial_approved',
  ...
  it('maps RPC error grant_ticket_approval.approval_not_found to NotFoundException', async () => {
    const { svc } = makeTicketService({
      approval: ticketApprovalRow(),
      rpcStub: () =>
        Promise.resolve({
          ...
          error: {
            code: 'P0001',
            message: 'grant_ticket_approval.approval_not_found: id=apr-1 tenant=T',
          },
        }),
    });

    // mapRpcErrorToAppError routes the registered code to a 404 AppError.
    await expect(
      svc.respond('apr-1', { status: 'approved' }, 'P-1', 'U-1'),
    ).rejects.toThrow();
  });

  it('rejects invalid p_decision values before hitting the RPC (response gate at controller)', async () => {
    // The controller's RespondDto type only allows 'approved'/'rejected';
    // the RPC's `invalid_response` raise is a defense-in-depth check.
    // Verify the TS layer surfaces an unexpected RPC error from that
    // path as a registered AppError code.
    const { svc } = makeTicketService({
      approval: ticketApprovalRow(),
      rpcStub: () =>
        Promise.resolve({
          ...
          error: {
            code: 'P0001',
            message: 'grant_ticket_approval.invalid_response: p_decision must be approved or rejected',
          },
        }),
    });
  ...
  it('returns resolved RPC outcome with full automation flags on approve', async () => {
    const { svc } = makeTicketService({
      approval: ticketApprovalRow(),
      rpcStub: () =>
        Promise.resolve({
  ...
  it('returns resolved RPC outcome with closed status on reject', async () => {
    const { svc } = makeTicketService({
      approval: ticketApprovalRow(),
      rpcStub: () =>
        Promise.resolve({
    ...
    const result = await svc.respond('apr-1', { status: 'rejected' }, 'P-1', 'U-1');
    expect(result).toMatchObject({
      kind: 'resolved',
      final_decision: 'rejected',
      ticket_status_category: 'closed',
