      ...
      {tasks.map((t, i) => (
        <div key={i} className="grid gap-1 border rounded p-2">
          ...
          <Input
            ...
            disabled={readOnly}
          />
          <Button variant="ghost" size="sm" onClick={() => setTasks(tasks.filter((_, j) => j !== i))} disabled={readOnly} className="gap-1">
            <Trash2 className="h-3.5 w-3.5" /> Remove
          </Button>
        </div>
      ))}
      <Button variant="outline" size="sm" onClick={() => setTasks([...tasks, { title: '' }])} disabled={readOnly} className="gap-1">
