...
- `AuthGuard` / `AdminGuard` / `PermissionGuard` / global tenant binding / any admin/config controller using `TenantContext.current()` / `PersonController` (`@Get('me')` ordering before `@Get(':id')`; `PersonService.getMe`) / any migration touching schema-wide GRANT/REVOKE EXECUTE or the RLS-helper functions (`current_tenant_id` / `current_user_id` / `user_has_permission`) / any migration touching `supabase_realtime` publication membership (add/remove a published table) or Realtime channel auth / RLS policies on a published table (`inbox_notifications` / `bookings` / `booking_slots` / `orders` / `order_line_items` / `recurrence_series` / `room_booking_rules` / `vendor_order_status_events`) → `pnpm smoke:cross-tenant` (also covers browser-path RLS-helper EXECUTE regression — the blanket-`REVOKE EXECUTE` / 00435-outage class: a real browser JWT doing plain PostgREST reads must return 200, not `42501 permission denied for function`; AND the R1 unwrapped-throw regression — browser-token `GET /api/persons/me` must return 200 with `id`, not 500 `unknown.server_error`; AND the R4 Realtime CDC path — a real browser JWT subscribing to `public.inbox_notifications` must receive a CDC event end-to-end within 15s, not silently drop on publication drift / per-subscriber RLS / Realtime-leg 00435-class regression)
