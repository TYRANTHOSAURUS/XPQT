...
- **Client: page primary queries use `usePageQuery`, not `useQuery`.** The `usePageQuery` helper auto-throws page-class errors (`not_found`/`permission`/`server`/`gone`) to `RouteErrorBoundary` so the page replaces instead of toasting over stale content. Sidebar / autocomplete / prefetch queries use `useQuery` + `handleQueryError(error, { callSite: 'mutation' })` from a `useEffect`. The distinction is made at the call site.
