...
>
> **SLA cron overlap guard (2026-05-21):** `SlaService.checkBreaches` now has an in-process reentrancy guard, covered by `sla.service.spec.ts`, so overlapping minute ticks skip instead of racing the same threshold pass. The live smoke intentionally remains a real-cron probe; no deterministic HTTP/test hook was exposed because the cron is already an ordinary service method in unit tests, and a production-accessible trigger would create a second mutation surface for SLA state. Verified 2026-05-21 against a Worker-backed API process: `smoke:tickets` **123 pass / 0 fail** including the real SLA escalation cron probe, and `smoke:work-orders` **125 pass / 0 fail**. Run `smoke:tickets` against an isolated API process when changing the SLA escalation path.
>
