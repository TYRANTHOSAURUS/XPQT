...
> **Audit-02 remediation (2026-05-16) — known live-probe gaps, explicit + owned.** The tickets/work-order audit remediation closed P0-1, P0-2, P1-1…P1-5 and shipped 2 RPC migrations (00406 `set_entity_assignment` v3, 00410 `update_entity_combined` v7, both pushed + remote function bodies verified). Those surfaces are **code-gated** (per-slice unit specs + `/full-review` adversarial pass; remote RPC bodies verified via `pg_get_functiondef`) but **not yet live-HTTP-smoked** — `smoke:work-orders` still does NOT cover: `PATCH /tickets/bulk/update` (P0-1); `POST /tickets|work-orders/:id/reassign` happy-path `command_operations` (P1-1); SLA-escalation reassign (P0-2); the routing-evaluation handler `routing_status` clear (P1-2); `getChildTasks` cross-visibility (P1-5); satisfaction round-trip (P1-3, no caller exists yet); vendor-assignment-through-orchestrator; WO cross-tenant sibling; dispatch idempotency-replay; reclassify. **Owner:** the audit-02 workstream's deferred Slice-8 probe set (see `docs/follow-ups/audits/02-tickets-work-orders.md` Closure Ledger "Slice 8 — live-smoke" row for the enumerated probes + per-finding risk). **Why deferred:** the shared `:3001` dev runtime was contended by a concurrent audit-03 session for the whole workstream (server-code-provenance unattributable, fixture-collision risk); authoring probes that cannot be run would be a paper-tiger guard. **Risk:** the 2026-05-01-class hazard (unit-green while a real-DB path regresses) is mitigated for the 2 pushed RPCs by direct remote body verification + per-caller backward-compat analysis, but the live HTTP→DB happy/replay paths of the closed surfaces remain unverified end-to-end until these probes run against an uncontended runtime.
