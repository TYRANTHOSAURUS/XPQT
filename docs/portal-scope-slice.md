...
> **Predicate-name update (2026-04-23, service-catalog collapse shipped):** the portal-facing predicates `portal_visible_request_type_ids` and `portal_availability_trace` described below were dropped in Phase E (migration `00097`). The shipped portal + simulator paths call the request-type-native replacements `public.request_type_visible_ids` and `public.request_type_requestable_trace`. Runtime semantics are unchanged; signatures differ (see [`docs/service-catalog-live.md §6`](./service-catalog-live.md)). All §4.6 / §4.7 / §8 prose in this file is historical — use the live doc for canonical names.
