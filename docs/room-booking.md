...
| `BundleCascadeService.cancelOrdersForReservation` | called by `ReservationService.cancelOne` and `RecurrenceService.cancelForward` per cancelled reservation | Looks up the bundle via `orders.linked_reservation_id` and cascades only orders + lines + tickets + asset reservations linked to that one reservation, rescoping (not voiding) shared approvals. Sibling occurrences in a recurring bundle stay alive. Best-effort — failures log but don't undo the reservation cancel |
| `BundleCascadeService.cancelBundleInternal` | reserved for explicit "cancel the whole bundle" callers (full cascade) | Visibility-bypass; voids every reservation, order, line, ticket, asset reservation, and pending approval scoped to the bundle. Used when scope is explicitly the whole bundle |
| `OrderService.cloneOrderForOccurrence` (rule re-eval) | inline within the clone | Per-occurrence service rules re-resolve so date-sensitive deny rules (e.g. "no catering on Dec 25") fire correctly per occurrence; `require_approval` outcomes route via `ApprovalRoutingService`; `deny` outcomes mark the cloned line `recurrence_skipped=true skip_reason='rule_deny'` and cancel any cloned asset reservation |
