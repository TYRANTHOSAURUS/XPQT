...
- ~~Scope-override resolver integration~~ — **shipped 2026-04-23.** `ScopeOverrideResolverService` + migration 00096 wire precedence (exact_space → ancestor_space (inherit) → space_group → tenant) into `ResolverService` (pre-step for handler), `TicketService.runPostCreateAutomation` (workflow + case SLA), `DispatchService.resolveChildSla` (executor SLA), and `RoutingEvaluatorService` policy-entity loaders. See [`docs/assignments-routing-fulfillment.md §24`](./assignments-routing-fulfillment.md).
