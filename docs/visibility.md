...
`public.tickets_visible_for_actor(p_user_id uuid, p_tenant_id uuid, p_has_read_all boolean default false)` (migration 00187) wraps the predicate in a `SETOF tickets` RPC so the API can chain PostgREST filters/sort/pagination directly on visible rows. This is the preferred path for set-based reads (`TicketService.list`, `listDistinctTags`) — it pushes the visibility join into Postgres instead of materializing the full visible-ticket-id set in Node and feeding it back as `.in('id', ids)`. The latter pattern is pathological for tenants with large visible sets (megabytes of UUIDs over the wire + a giant IN list for the planner). When `p_has_read_all = true` the wrapper short-circuits the predicate join entirely.

**`getChildTasks` — per-child work-order visibility (audit 02 / P1-5):** `TicketService.getChildTasks` does NOT use `tickets_visible_for_actor` for children. Instead it: (1) gates on parent-case readability via `assertVisible(parentTicketId, ctx, 'read')`, then (2) fetches all child work-orders for the parent from `work_orders`, then (3) filters the fetched rows through `TicketVisibilityService.getVisibleWorkOrderIds(ctx)` — which calls `public.work_order_visibility_ids` (migration 00374). Seeing the parent case does **not** grant visibility on all its child work-orders; each child is evaluated independently. This closes the prior leak where a vendor WO dispatched outside the actor's scope was visible if the actor could see the parent case.
...
| `getVisibleWorkOrderIds(ctx)` | Same contract as `getVisibleIds` but calls `public.work_order_visibility_ids` (migration 00374). `null` = see all (has `tickets:read_all` — WO inherits the same bypass by design, 00374 §108-114). `[]` = no user. `string[]` = filtered id set. Used by `getChildTasks` to filter child WOs independently of parent-case visibility (audit 02 / P1-5). |
