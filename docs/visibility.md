...
- **Child work_order visibility — resolved 2026-05-16 (Audit 02 / P1-5).** `TicketService.getChildTasks(parentCaseId)` previously **inherited parent visibility**: any actor who could `read` the parent case received *every* child work_order on it, regardless of each child's own assignee / vendor / team / role scope — a requester who could see a case also saw a child WO dispatched to a sensitive vendor. Now: parent-case `read` is a **precondition** (you can't enumerate a case's children without seeing the case), but each child is additionally filtered through the work_order visibility predicate `work_order_visibility_ids` (00374) — the actor only sees the children individually visible to them. `tickets:read_all` and the `SYSTEM_ACTOR` bypass the per-child filter (override / internal). Empty visible set ⇒ the actor sees the parent but none of its children. This was a real read-side leak the doc was previously silent on (audit doc-drift §7). **Deliberate decision (not a side effect):** `tickets:read_all` — a case-level admin override — *intentionally* also grants unfiltered child-WO visibility, per the 00374 model where the same permission key gates both `tickets` and `work_orders` (`work_orders_visible_for_actor` honours the same `has_read_all`). A `read_all` holder therefore still sees every child WO regardless of the child's assignee/vendor/team — acceptable as an admin override, but noted as a residual: if WO vendor-sensitivity ever needs to be opaque even to case-level `read_all`, the lever is a distinct `work_orders.read_all` permission key, not the current shared key. **Sanctioned privileged aggregate (Audit 02 / P1-5 FE-rollup, 2026-05-18).** The P1-5 list-filter (correctly) made the desk sub-issue progress ring/badge under-report ("2 of 3") for a scoped operator — the route guard is the coarse `requiredRole="agent"`, **not** `tickets:read_all` — because the count was computed client-side from the filtered child array. `TicketService.getChildTasksRollup(parentCaseId)` / `GET /tickets/:id/children/rollup` is the decoupled fix: it runs the **identical precondition** as `getChildTasks` (same context-load + `assertVisible(parent, 'read')`, same 403/404 for an unreadable/nonexistent parent), then a **tenant-scoped** (`tenant_id` predicate mandatory — #0 invariant — even though `supabase.admin` bypasses RLS) privileged count over `work_orders WHERE parent_ticket_id = …` that **deliberately omits** the per-child `work_order_visibility_ids` filter and returns **only** `{ done, total }` (`done` = `status_category in ('resolved','closed')`). No child id / title / assignee / vendor / priority / SLA / timestamp ever crosses the boundary — only two integers. **Disclosure boundary (stated honestly — this is NOT a wholesale equivalent of the tenant-wide reporting-service precedent).** The reporting-service precedent is a *tenant-wide* aggregate where no single subject is inferable; this rollup is *per-parent*, so it is a deliberately-accepted **bounded narrowing** of P1-5's row-hide, not zero-disclosure. A parent-`read`-able actor learns the **existence and aggregate done-count of children they cannot individually see**. At small N this sharpens: with `total=1, visible=0, done=1` the actor infers the case has exactly one (hidden) child **and** that it is resolved/closed — the lifecycle state of one specific hidden work_order. The set that passes `assertVisible(parent,'read')` includes the **requester/watcher participant class** (read is the broad floor), so at the *endpoint* level this re-exposes child *existence + done-state* to the very requester class P1-5's row-hide protected (the desk FE route additionally redirects non-`agent` roles to the portal, so the practical FE surface is the scoped desk operator; the requester exposure is the direct-API edge). **What remains protected (P1-5's core, intact at every N):** child identity, title, assignee, **vendor** (the sensitive "which vendor" signal P1-5 exists to guard), priority, SLA, timestamps. **Why accepted (codex-design-checked + product-owner-approved 2026-05-18):** the rollup's entire purpose is to give a scoped operator the *true* progress of work on a case they legitimately oversee — flooring it (showing only "has hidden sub-issues") would re-introduce the exact under-report defect being fixed; and existence + done-count is categorically weaker than P1-5's protected signal (which-vendor / which-WO). The acceptance is explicit and bounded — NOT an "equivalent-to-reporting, no leak" claim. **Residual lever (tracked, not a current defect):** if the requester-class direct-API exposure of child existence/done-state is ever deemed unacceptable, gate the rollup endpoint on an operator-tier permission (team-member / role-scope / `tickets:read_all`) instead of the broad parent-`read` floor. The FE single-sources the progress ring + the sub-issues header count from this rollup; the child *list* still renders the `work_order_visibility_ids`-filtered set unchanged, with a muted "Some sub-issues may be hidden by visibility rules." tooltip when the visible list is shorter than the privileged total.
