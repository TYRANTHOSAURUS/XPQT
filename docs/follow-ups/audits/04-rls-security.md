...
- **Scope of this probe (honest).** This browser-path probe covers the **REST browser path + the `authenticated` role** for the `42501` / blanket-`REVOKE EXECUTE` class; the **`anon` EXECUTE leg** is covered by the static `rlsHelperExecutePosture` grant check (now overload-safe — any non-EXECUTE-able overload of an overloaded helper is flagged, and a missing-entirely helper surfaces), and the Supabase **Realtime channel path is NOT exercised end-to-end** (known limitation, acceptable: REST is the representative browser read path, and the static grant probe + the `tickets_distinct_tags` foreign-tenant leak probe cover the adjacent axes). **2026-05-20 R3 precision fold:** non-200 outcomes on the browser-path RLS read are now labeled distinctly across three classes — `42501-rls-helper` (the catastrophic 00435-outage class: body contains `permission denied for function` OR JSON `code === '42501'`), `postgrest-4xx` (HTTP 4xx with a JSON envelope whose `code` is NOT `42501`; surfaces the PostgREST `.code` — unrelated to RLS-helper EXECUTE), `transport-or-5xx` (HTTP >= 500, non-JSON body, or fetch threw). Fail-closed binary outcome preserved; only the failure label is precise now. Implementation: `apps/api/scripts/smoke-cross-tenant.mjs:1146-1310`. Mirrors the same three-class vocabulary `smoke:prod-e2e` (R5) ships so the failure surface across the two prod-adjacent gates speaks one language. **2026-05-20 R4 fold:** the prior clause "**Realtime channel path is NOT exercised end-to-end** (known limitation, acceptable)" is SUPERSEDED for the `inbox_notifications` publication. `smoke:cross-tenant` now ships a `realtimeChannelProbe` (named function near `mintAdminToken` in `apps/api/scripts/smoke-cross-tenant.mjs`, called from the IIFE main after the REST RLS probe loop) that opens a real WebSocket under a fresh ADMIN browser JWT, subscribes to `postgres_changes` on `public.inbox_notifications` (publication-included per 00401), service-role INSERTs a per-run unique fixture row (`event_kind = 'r4-probe-realtime-cdc'`), and asserts CDC event arrival within 15s (warm path P99 < 1s across 7 validation runs: 181-775ms; the 15s ceiling absorbs broker-provisioning jitter without masking real outages). Four named failure classes mirror the R3 three-class vocabulary so the gate speaks one language — `realtime-channel-subscribe-failed` (WS handshake never reached SUBSCRIBED), `realtime-cdc-timeout` (subscribed but no event in 15s — Realtime-leg RLS-helper EXECUTE regression OR publication drift OR replication-slot stall OR Realtime backend outage), `realtime-payload-mismatch`, `realtime-leak-foreign-tenant` (defense-in-depth: under correct RLS this can't fire, but the label exists so a future cross-tenant Realtime leak surfaces loudly). The probe scopes a `tenant_id` claim onto ADMIN's `raw_app_meta_data` immediately before minting the JWT and unconditionally strips it in `finally` (Supabase Auth's `updateUserById({ app_metadata })` MERGES rather than replaces; the restore sends `{ tenant_id: null }` to actually delete the key, which is both deterministic on success and idempotently self-healing if a prior run crashed mid-probe). Per-run fixture UUIDs + `finally` teardown — the post-run audit `select count(*) from public.inbox_notifications where event_kind like 'r4-probe-%'` MUST be 0 (verified across 7 runs). The wider Realtime surface (other published tables: `booking_slots`/`bookings`/`order_line_items`/`orders`/`recurrence_series`/`room_booking_rules`/`vendor_order_status_events`) is NOT exercised here — their RLS predicates also call `current_tenant_id()`, so a Realtime-leg `42501` would surface on `inbox_notifications` first; the publication-membership static check on the broader set continues via the existing `selectMissing` browser-grant probe.
