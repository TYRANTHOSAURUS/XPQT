...
- **[code-review C2 — REFUTED, valid kernel now CLOSED] "00415 is a no-op; grants were already zero; red→green is tautological."** Refuted: no migration before `00415` performs a broad table-DML revoke on the escalation tables (the other `revoke` migrations — 00274/00280/00282/00378/00394/00395/00407 — revoke *functions*). The contemporaneous pre-push smoke output (`✗ 12 anon/authenticated write grants still present`) → post-push (`✓ … NO INSERT/UPDATE/DELETE`, `esc_writes_left=0`) is the authoritative red→green. The reviewer queried the live DB **after** this session's mid-review push and misread the post-fix state as the original. **Valid kernel — CLOSED:** the change set is now committed (user-authorized) as **`89f3e9ae`** (file-scoped: the 5 RLS files + this ledger; parallel-workstream files excluded). Verified against **committed HEAD `89f3e9ae`** (not the working tree — the audit's recurring false-green failure mode): `git show HEAD:notification.controller.ts` has only the surviving template routes (zero IDOR routes); `git show HEAD:notification.service.ts` has zero IDOR methods; `00415` is in `git ls-tree HEAD`; `git grep "@UseGuards(AdminGuard)" HEAD` still zero; site-6 repoint present in committed tree; live remote `writes_left=0` across all public tables = remote matches committed migration. The remote-vs-committed divergence is resolved.
...
> **[SUPERSEDED — see the next block "codex done-check — found a LIVE cross-tenant RPC leak".]** The "RPC-EXECUTE = tracked-P2, NOT hardened" framing in this paragraph was WRONG: codex's done-check found `tickets_distinct_tags` was a real LIVE cross-tenant leak (not latent). It is now **closed** via the corrected `00417` (revoke EXECUTE from PUBLIC + re-grant service_role), verified live + smoke 42/0 + work-orders 109/0. Read the next block for the authoritative #2 status. Browser-direct *reads* (SECURITY INVOKER, RLS-gated, claim-dependent) remain the only tracked residual on this axis.
...
Commit: **`89f3e9ae`** on `feature/booking-audit-remediation` (user-authorized, file-scoped to the 5 RLS files + this ledger; the parallel audit02 workstream's dirty files deliberately excluded). Security invariants re-verified post-commit against committed HEAD `89f3e9ae` + live remote (not the working tree). Branch still not merged to `main` (gated on the parallel 03-booking workstream, unchanged — independent of this audit's closure).

#### Update — 2026-05-18 (codex done-check — found a LIVE cross-tenant RPC leak; the "RPC=tracked-P2, not live" framing was WRONG; closed via corrected 00417)

A user-requested codex done-check (read-only, vs committed HEAD) returned **NOT-DONE** and was correct. It found a concrete LIVE cross-tenant leak that the full-review-block's "RPC-EXECUTE = known/assessed/tracked-P2, not live" framing wrongly downgraded:

- **The leak (verified independently, static + live):** `public.tickets_distinct_tags(tenant uuid)` (`00031_tickets_distinct_tags_rpc.sql`) is `SECURITY DEFINER`, `set search_path=public`, body `select distinct … from tickets where tenant_id = tenant` — it trusts the **caller-supplied `tenant` arg** with NO `current_tenant_id()` / `auth.uid()` / membership check, and **no later migration revokes/guards it**. Live proof: a real authenticated TENANT_A browser session token `POST {SUPABASE_URL}/rest/v1/rpc/tickets_distinct_tags {tenant:<foreign>}` → **HTTP 200** returning that tenant's distinct ticket tags. Reachable NOW, claim-independent (it trusts the arg, not the unminted JWT claim). Low data sensitivity (free-text tag labels — but can include site/location names) yet a categorical tenant-isolation violation = **P0-class per the #0 invariant**, not the "P2/not-live" the prior block asserted. **That prior framing is withdrawn/corrected here.** Root: Supabase's platform default `GRANT EXECUTE … TO anon/authenticated` (370 public fns) — the function-level twin of the 00415 table hole.

- **First `00417` push was INEFFECTIVE — verification caught it, no false closure claimed.** v1 did `revoke execute … from anon, authenticated`. Post-push verify showed `tickets_distinct_tags` STILL `authenticated=true` and the live probe STILL 200. Root cause (confirmed via `proacl = {=X/postgres,…,service_role=X/postgres}`): Postgres' `CREATE FUNCTION` default grants EXECUTE to **PUBLIC**; there was never an `anon/authenticated`-specific ACL, so revoking from those roles was a no-op and `authenticated` kept EXECUTE **via PUBLIC**. (This is exactly the verification-before-completion discipline working — the pushed migration was NOT claimed to have closed the leak.)

- **Corrected `00417` (the audit's own canonical SECURITY DEFINER pattern):** `REVOKE EXECUTE ON ALL ROUTINES IN SCHEMA public FROM public, anon, authenticated` + `GRANT EXECUTE ON ALL ROUTINES IN SCHEMA public TO service_role` (the API role — `SupabaseService.admin`; raw-pg `DbService` connects as `postgres`, the function OWNER, inherently retains EXECUTE; trigger-function execution does not check EXECUTE so triggers are unaffected) + re-grant the audit-documented anon-callable bearer-token trio. ADP also revokes EXECUTE on routines from public/anon/authenticated (partial — `postgres`-created only, same honest caveat as 00415). Pushed (user-authorized).

- **Verified (static + live + smoke; API blast-radius checked):** post-push — `tickets_distinct_tags` anon=`false` authenticated=`false` service_role=`true`; **0** postgres-owned public routines browser-EXECUTE-able beyond the trio; trio = 3 preserved; `service_role`+`postgres` retain EXECUTE (API path). Live probe: own-AND-foreign-tenant `tickets_distinct_tags` → **HTTP 403 `42501 permission denied`** (browser-direct denied; the app calls it via the API/service_role). `pnpm smoke:cross-tenant` → **42 pass / 0 fail** (new: EXECUTE-guard GREEN, `tickets_distinct_tags` foreign → 403, trio-preserved). `pnpm smoke:work-orders` → **109 / 0** — the decisive blast-radius check: the broader PUBLIC-revoke + service_role re-grant did NOT break the API path.

- **Smoke guard precision (no paper tiger — `[[feedback_runnable_guards_mandate]]`):** `browserExecuteGrantPosture()` is scoped to `proowner = postgres` (app/migration routines). The ~219 still-browser-executable routines are `supabase_admin`-owned `pg_trgm`/`btree_gist` math (zero tenant data) that the non-owner migration role CANNOT revoke and that `similarity()`-based search needs — asserting "zero of ALL routines" would be an unachievable gate. Every SECURITY DEFINER tenant-data function (incl. the proven leak) is `postgres`-owned; "zero postgres-owned app routines browser-executable except trio" is the achievable, security-meaningful invariant, and it is met (0).
...
#### Update — 2026-05-19 (00417 caused a PRODUCTION OUTAGE; reverted by 00420 / PR #31; browser-path RLS smoke probe added so this class is caught)

The corrected `00417` from the block above (`REVOKE EXECUTE ON ALL ROUTINES IN SCHEMA public FROM public, anon, authenticated` + `GRANT … TO service_role` + bearer-token-trio re-grant) was **catastrophically wrong** and broke the live app. Append-only correction of record — the block above is not rewritten; this block governs where they conflict.

- **Incident (2026-05-19, production, self-caused, user-caught).** After `00417` reached the remote, every logged-in user saw all data vanish. Browser `GET /rest/v1/<any RLS table>` and Supabase Realtime reads failed with `42501 permission denied for function current_tenant_id`. The "narrow per-function lock" framing missed that `REVOKE … ON ALL ROUTINES` also revoked EXECUTE on the RLS *helper* functions (`public.current_tenant_id()`, and similarly `current_user_id()` / `user_has_permission()` / `gdpr_caller_has()`).

- **Mechanism.** Every `tenant_isolation` RLS policy's `USING`/`WITH CHECK` clause invokes `public.current_tenant_id()`. Postgres checks EXECUTE on a function **as the querying role** (`anon`/`authenticated`) — **even for `SECURITY DEFINER`** (DEFINER switches the role *inside* the body, not the EXECUTE permission check at call time). With browser-role EXECUTE revoked on the helper, every RLS-protected read by a logged-in browser/Realtime session 42501'd. The NestJS-API smoke gates never caught it because they run on the `service_role` path, which retained EXECUTE and bypasses this check; the then-existing browser smoke probes only asserted *denial* and mis-read the 42501 as success rather than testing a normal authenticated RLS read as a regression.

- **Fix — migration `00420` (via PR #31, merged at `7cf354c9`).** Full revert of `00417`: `GRANT EXECUTE ON ALL ROUTINES IN SCHEMA public TO anon, authenticated` + `ALTER DEFAULT PRIVILEGES … GRANT EXECUTE … TO anon, authenticated` (restores the Supabase default that RLS depends on), then re-applies **only** the narrow, RLS-safe per-function lock for the one codex-found live leak: `REVOKE EXECUTE ON FUNCTION public.tickets_distinct_tags(uuid) FROM public, anon, authenticated` + `GRANT … TO service_role` (a standalone `SECURITY DEFINER` RPC not referenced by any RLS policy and not called browser-direct by the app). `00415` (table-DML write revoke, SELECT kept) did **not** cause the incident and is unchanged. The broad RPC-EXECUTE surface returns to its pre-session state — **tracked-P2 as originally framed; the blanket-close is infeasible and is withdrawn**. The remote DB was hand-restored to this exact state during the incident; `00420` is the idempotent authoritative record.

- **Live verification (remote, post-00420).** Browser session token (real magiclink→verify path) `GET /rest/v1/{inbox_notifications,bookings,tickets}?select=id&limit=1` → **HTTP 200**, no `permission denied for function` (42501 gone). `tickets_distinct_tags(<foreign tenant>)` browser-direct → **HTTP 403** (narrow lock still holds). Migrations `00415` / `00417` / `00420` all applied to remote. `pnpm smoke:cross-tenant` → **45 pass / 0 fail** (2 consecutive runs); `pnpm -C apps/api lint` + `pnpm errors:check-app-errors` green.

- **Lesson + probe added (this PR).** **Never schema-wide `REVOKE EXECUTE` on `public` functions from `anon`/`authenticated` in a Supabase project — it breaks RLS** (RLS policies call helper functions evaluated as the querying role even for `SECURITY DEFINER`). Lock only proven `SECURITY DEFINER` leaks **per-function**. Root coverage gap: every smoke gate used the `service_role`/NestJS-API path, which bypasses the RLS-helper EXECUTE check, so the outage shipped undetected. **Fixed in this PR:** `apps/api/scripts/smoke-cross-tenant.mjs` now runs a dedicated **browser-path RLS regression probe** — folded into `pnpm smoke:cross-tenant` — that mints a real authenticated browser session token (the existing `mintTokenFor` magiclink→verify helper) and does a plain `GET /rest/v1/{inbox_notifications,bookings,tickets}` directly against Supabase REST, failing the gate on any non-200 OR a `permission denied for function` body. The now-incorrect 00417-era "zero browser-EXECUTE-able app routines" assertion (it encoded the catastrophic state and went RED against the green post-00420 `main` — a broken gate per the runnable-guards mandate) is replaced with its RLS-correct inverse: the RLS-helper trio MUST be browser-EXECUTE-able, while the narrow `tickets_distinct_tags` lock probe is retained (still 403).
...
Completion bar status: **[SUPERSEDED — read the two later blocks: "full-review — corrections of record" AND "codex done-check — found a LIVE cross-tenant RPC leak". Authoritative #2 status: browser-direct table-DML (00415) AND RPC-EXECUTE (00417) BOTH grant-denied/claim-independent; the only tracked residual is browser-direct SECURITY INVOKER reads (RLS-gated, claim-dependent, SELECT kept for Realtime). The wording below understated table-DML and wrongly downgraded a live RPC leak to P2.]** no cross-tenant header flip or same-tenant privilege escalation remains in the API surface (Slices 1–11); notification read-state can no longer be changed across users (item #1 fixed); browser-direct **table-DML** posture hardened (item #2 — `00415` write-grant revoke) — RPC-EXECUTE now ALSO closed via `00417` (codex-found live leak); avatar Storage cross-tenant READ remains a tracked P3. Verified against committed-HEAD code + live remote DB + the running API. The `/full-review` gate ran (findings folded in via the corrections block); commit is file-scoped to the 5 RLS files (tree-`tsc`-red is exclusively the parallel workstream).
