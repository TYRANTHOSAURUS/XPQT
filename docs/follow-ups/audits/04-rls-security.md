...
| 2026-05-14 | Slice 6 — deny-by-default RLS on `public.tenants` (P2) | **awaiting push** | Migration `supabase/migrations/00405_tenants_deny_by_default_rls.sql`. `ALTER TABLE public.tenants ENABLE ROW LEVEL SECURITY` + `CREATE POLICY tenants_deny_all_by_default USING (false) WITH CHECK (false)`. Cross-checked all `from('tenants')` callers: 15 callsites under `apps/api/src` plus 4 migrations, ALL use `supabase.admin` (service-role) or run as `postgres` superuser — both bypass RLS, so this policy is a runtime no-op for current paths. Closes the audit's P2 finding (`04-rls-security.md:71-74`). | Committed locally; not yet pushed to remote. Verification will run after push: `select * from public.tenants` as service-role still returns rows; as a future non-superuser role would return zero. | Per the migration-collision memory, claimed `00405` at write time. If a parallel workstream takes that slot, bump on push. |
