...
| 2026-05-16 | Should-fix #19 — satisfaction rating into `update_entity_combined` metadata branch (Agent 2 P1-3) | **CLOSED (code + remote migration verified; smoke → Slice 8/when-caller-exists)** | See `docs/follow-ups/audits/02-tickets-work-orders.md` Closure Ledger row P1-3 + inline Update block (2026-05-16). Migration `00410_update_entity_combined_v7_satisfaction.sql` (renumbered from 00407 — cross-session triple-claim) **pushed to remote + verified** (v7-sat=t, case-only-guard=t, v6-plan-lock=t). Commits 087e7ed9 + 9f2c612f + 78f8ea8a. | tsc + errors:check-app-errors green; WO 39/39 + ticket 23 specs; `/full-review` 2-agent both SHIP (Plan-2 + Plan-1 folded); remote body verified v7. codex unobtainable (concurrent-session contention) — gate = /full-review + per-caller backward-compat + verified remote body. | v7 keys-absent → byte-identical to v6 (5 callers unaffected). Case-only (WO+sat raises). `tickets.satisfaction_rating` documented legacy vs shipped requester-rating (separate table) — no caller exists today. Residuals: cross-session 00407 collision → P2-3; Plan-C1 detection in place; no dedicated satisfaction spec (transitive). |
| 2026-05-16 | Should-fix #17 — `getChildTasks` filter children through `work_order_visibility_ids` (Agent 2 P1-5) | **CLOSED (code + spec; FE rollup follow-up deferred)** | See `docs/follow-ups/audits/02-tickets-work-orders.md` Closure Ledger row P1-5 + inline Update block (2026-05-16). TS-only (no migration). Commits 6b4af8cd + 85dc82d6. | tsc + errors:check green; new `ticket-get-child-tasks.spec.ts` 5/5; `/full-review` code agent independently verified security-correct / fail-closed / NO leak. codex unobtainable — gate = /full-review + 5-case spec + green gates. | Parent-case read = precondition; each child filtered via WO predicate; `tickets:read_all`/SYSTEM bypass (documented as deliberate per 00374 same-key model). Deferred (FE, brief scope): `SubIssueProgress` badge under-reports for scoped-out actors → FE follow-up (server-side privileged rollup or per-actor label). Cross-visibility live smoke = Slice 8. |
| 2026-05-16 | Should-fix #16 — service-layer case-vs-WO split (Agent 2 P2-1) | **DEFERRED (explicit rationale)** | See audit-02 inline Update + Closure Ledger row P2-1. P2 hygiene, "probably a multi-day refactor", not a completion-bar item; security/atomicity consequences already closed in Slices 1–6. No code change. | N/A (deferral). Scope = direction-class; codex unobtainable all workstream. | Owner = integrator/data-model. Cheap interim recommended: reject WO ids on `PATCH /tickets/:id`. |
| 2026-05-16 | Blocker #8 — duplicate migration prefixes (Agent 1 P0-1 / Agent 2 P2-3) | **DEFERRED to this blocker's owner + NEW EVIDENCE + audit-02 self-mitigation** | audit-02 observed `00407` **triple-claimed live** across this/audit-03/phase-1.5 (disjoint fns, no clobber — `git log --all` verified); rebased its own Slice-5 migration `00407→00410`; `00406` already unique. No historical renumber (cross-session-unsafe from one worktree). See audit-02 inline Update P2-3. | `00406`+`00410` unique on-disk + git-log-all; both remote-verified. | The historical renumber + `scripts/check-migration-prefix-unique.sh` CI guard remain owned by this blocker's integrator/data-model owner — collision is active in `00406+`, not just `00367–00400` (now demonstrated). |
| 2026-05-16 | **audit-02 workstream — COMPLETE** | **All P0 + all P1 CLOSED; P2-4/P2-5 closed; P2-1/P2-3 deferred (rationale); P2-2 partial; P3 triaged; live-smoke deferred (owner+risk)** | Specialist audit `02-tickets-work-orders.md` driven to closure on `feature/tickets-wo-audit-remediation` (isolated worktree). Migrations 00406 + 00410 pushed to remote, function bodies verified. All ledgers (02 + this 00) reconciled append-only. | Per-slice tsc/`errors:check-app-errors`/jest green; `/full-review` adversarial pass every slice; remote RPC bodies verified by `pg_get_functiondef`; codex tertiary gate unobtainable the entire workstream (concurrent-session resource contention) — gate = `/full-review` + targeted self-verify + verified remote bodies + green gates/specs. | Completion bar met: no P0 raw-write bypass · assignment paths canonical/atomic-or-documented · visibility code-covered (smoke deferred w/ owner+risk) · reference docs synced (`visibility.md`/`assignments-routing-fulfillment.md`/`b2-followups.md`/`smoke-gates.md`). Branch ready for the merge decision. |
| 2026-05-17 | **audit-02 best-in-class continuation** — Should-fix #16 interim + codex gate + Code-I1 | **P2-1 interim SHIPPED · codex tertiary gate OBTAINED (was environmentally unobtainable) · Code-I1 RE-DEFERRED w/ prescription · live-smoke → see audit-02 §2026-05-17 row** | Continuation off `origin/main` 34f82c0a (isolated worktree). P2-1 WO-id reject guard shipped (aac61b7a); codex Q1–Q4 obtained — 00406/00410 **safe-as-merged for all callers**, Q3 clean, Q4 re-defer confirmed, 3 NITs (2 folded 53ea0c66, 1 documented forward-only). Code-I1 racy-TS-guard reverted; re-deferred with a codex-validated partial-unique-index prescription + owner + risk. See `02-tickets-work-orders.md` Closure Ledger 2026-05-17 rows + §2026-05-17. | `/full-review` 2-agent per slice + **codex now responsive** (prompt-to-file mitigation); tsc/errors:check/web-tsc green per slice. | Should-fix #16 interim closes the misleading-error sharp-edge (full split still owned here). The 2026-05-16 "codex tertiary gate unobtainable" caveat across THIS verdict's audit-02 rows is now CLOSED — codex was obtained and the merged RPC surface is codex-clean. Cross-session items (PR#16 CI B.2-config-reads RED; P2-3 renumber; P1-5 FE rollup) routed, NOT absorbed: `docs/follow-ups/audit-02-best-in-class-routing-2026-05-17.md`. |
| 2026-05-17 | **audit-02 — BEST-IN-CLASS bar MET** (live-smoke discharged) | **The 2026-05-16 "live-smoke deferred (owner+risk)" — the single biggest gap per the final-recommendation thesis ("best-in-class architecture ≠ best-in-class product") — is now CLOSED** | 10-probe live-API set covering every 2026-05-16 P0/P1 closure, registered in the CLAUDE.md mandatory matrix + `smoke-gates.md`, **independently re-run green by the orchestrator twice post-fold** (`smoke:tickets` 122/0, `smoke:work-orders` 125/0, exit 0, zero CONTENTION-DEFER on 3 runs). Commit 051bbbe8 on `worktree-audit-02-best-in-class`. The brief's "CI RED on B.2 config-reads" premise was disproven by evidence (`b2:check-config-reads` is green on 34f82c0a) and the ACTUAL reds correctly re-attributed + routed. | Adversarial vacuousness review (no CRITICAL/fake-green; P1-5 probe proven to fail on revert); 2 IMPORTANT folds; 3 independent green runs; no genuine product regression. | Per the integrator final-recommendation thesis, "best-in-class" is now claimable for audit-02's scope: architecture-closed (2026-05-16) **and** product-verified by the project's own ship gate (2026-05-17). Residual deferrals (Code-I1 unique-index, P2-1 full split, P2-3 renumber, P1-5 FE rollup) are explicitly owned + risk-stated, not audit-02 gaps. |
| 2026-05-16 | Top-10 #2 / must-fix #2 — booking edit empty linked-row patches (Agent 3 P0-2/P0-3) | closed | assemble-edit-plan.service.ts buildLinkedRowPatches; migration 00407; smoke-edit-booking.mjs Fixture D; see audit 03 ledger 2026-05-16 | pnpm smoke:edit-booking 78/78 exit 0; jest src/modules/reservations 265/265 (255 P0-2 + 5 I-2 + 5 I-3 fail-closed) | Multi-slot residual deferred (loud warn). Cross-RPC idempotency scope confirmed booking-edit-only, NOT an integrator-wide P0. |
| 2026-05-16 | Discovered P0 (not in original verdict) — edit_booking.actor_not_found broke ALL editOne/editSlot/editScope since 2026-05-12 | closed | reservations dto/types.ts + reservation.controller.ts + reservation.service.ts + recurrence.service.ts + reservations.module.ts; see audit 03 D-1 | smoke A/B/C actor_not_found→pass; 78/78 | Contests audit 08 "editOne/editSlot smoke ✅ 2026-05-13". Approval/cancel path may share the class — flagged for booking Slice 2. |
| 2026-05-16 | Discovered P0 — RPC idempotency hash covered nondeterministic server fields (spurious payload_mismatch 409 on retries) | closed | migration 00407 (strip+hash helpers; edit_booking/edit_booking_scope verbatim, 1 line each); assemble-edit-plan.service.ts 6-array canonicalization; idempotency guard; see audit 03 D-2 | smoke idempotency-replay pass; 78/78; remote pg_get_functiondef verified | Forward-only; booking-edit scope only. |
| 2026-05-16 | Discovered P0 (D-3, cross-workstream) — `grant_ticket_approval` actor mismatch: `grant_ticket_approval` v3 (`00358:112` resolves `where u.auth_uid = p_actor_user_id`) requires auth_uid, but `approval.service.ts:737` passes `respondingUserId` (= users.id) | **OPEN — owner: tickets-WO (audit 02) workstream** | Found during booking Slice 2 plan-gate (codex). Same class as booking D-1 (`edit_booking.actor_not_found`). NOT fixed here — out of booking-audit scope; ticket-approval grant path. | Not run (discovered via static trace `00358:112` + `approval.service.ts:737`) | If real, EVERY ticket-approval grant 404s `grant_ticket_approval.actor_not_found` like booking D-1 did. tickets-WO workstream must verify + fix on `feature/tickets-wo-audit-remediation`; mirror booking D-1 fix (thread auth_uid into the approval actor). Booking Slice 2 deliberately does not touch this. |
| 2026-05-17 | Top-10 #3 / must-fix #3 — booking cancel non-atomic + missing `booking.cancelled` producer (Agent 3 P0-1 + P1-5) | closed | `migration 00408 cancel_booking_with_cascade` (atomic one-tx cascade + emits booking.cancelled per booking, on remote); cancelOne wrapper; durable BookingCancelledCascadeHandler; see audit 03 ledger 2026-05-17 + `docs/follow-ups/cancel-booking-equivalence-checklist.md` | pnpm smoke:cancel-booking 138/0; smoke:edit-booking 78/78 regression; jest reservations 265/265; errors+tsc clean; 00408 pg_get_functiondef verified | 2-agent full-review: 4 critical + 2 important all fixed+re-verified. Codex big-step hung 0-byte → skipped per protocol (self-review+empirical covered). P1-5 closed via existing wake handler. Residual I-1 (dead-letter backstop) deferred → owner outbox/infra. |
| 2026-05-17 | Must-fix #15 — multi-room create → `create_booking_with_attach_plan` (Agent 3 P1-1); retires `BookingTransactionBoundary` from multi-room | closed | `multi-room-booking.service.ts` atomic-RPC cutover + `migration 00410` (discovered D-4: validator §7a `applied_rule_ids`→`room_booking_rules`, also fixes single-room create-with-services latent 400) + new `smoke:create-multi-room`; see audit 03 ledger 2026-05-17 + `docs/follow-ups/slice3-multiroom-validator-decision.md` | smoke:create-multi-room 46/0; jest reservations 267/267; smoke:cancel-booking 138/0 + smoke:outbox 13/0 regression; errors+tsc clean; 00410 pg_get_functiondef verified | Option B (single-room booking-flow untouched). 2-agent full-review: 1 blocker (stale dishonest smoke-gates doc) + 4 findings all fixed. Codex 0-byte hung → skipped per protocol. recurrence-clone txBoundary caller remains (P2-1). R-a/R-b/R-c residuals documented. |
| 2026-05-17 | Must-fix #14 — recurrence split → `split_recurrence_series` RPC (Agent 3 P1-2) | closed | new `migration 00411_split_recurrence_series.sql` (atomic idempotent, canonical 00408 pattern; on remote, pg_get_functiondef verified) + `splitSeries` thin-wrapper + `skipSplitSeries` TS suppression hack deleted; see audit 03 ledger 2026-05-17 + `docs/follow-ups/slice4-split-recurrence-decision.md` | smoke:edit-booking-scope 45/1 (7 split probes green; 1 = pre-existing D-5/R-e, NOT split); jest reservations 265/265; smoke:edit-booking 75/1 (NOOR flake) + create-multi-room 46/0 regression; errors+tsc clean | TS idempotency-suppression eliminated (now in RPC command_operations gate). I1 system-actor uuid landmine fixed. **Discovered D-5** (deferred, task #14): pre-existing `edit_booking_scope` same-body-retry 409 — non-`_`-prefixed scope-PRODUCER determinism (NOT a missing-strip-helper one-liner: live-DB verified edit_booking_scope already uses booking_edit_idempotency_payload_hash via 00407:1256). A wrong root-cause the orchestrator propagated from an unverified reviewer claim was caught by an honest fix-subagent refusal + falsified vs ground truth before any no-op migration/false gate/wrong doc shipped — recorded in the 03 Slice-4 Update process-honesty note. Codex 0-byte hung → skipped per protocol. |
| 2026-05-17 | Must-fix #15-area — post-create service attach → `attach_services_to_existing_booking` RPC (Agent 3 P1-3) | **partial (code-complete+verified; live smoke + full-review PENDING — task #15)** | new `migration 00412` on remote (create-attach-half minus booking/slots; attach_operations idempotency; atomic) + bundle.service.ts cutover + TS `Cleanup` class deleted + controller clientRequestId; see audit 03 ledger 2026-05-17 + `docs/follow-ups/slice5-attach-services-decision.md` | tsc clean; jest booking-bundles+reservations 32 suites/333 passed; errors clean; 00412 pg_get_functiondef verified; validator-input correctness VERIFIED vs 00303. NO live smoke run | **HONEST PARTIAL — not closed.** Only booking-audit slice shipping without its live smoke gate + 2-agent full-review: BOTH authoring attempts killed mid-run by API infra failures (rate-limit; stream-idle-timeout) 2026-05-17. Tracked task #15; risk disclosed in decision doc, NOT silent, NOT a faked gate. RPC mirrors the already-smoke-gated shipped create attach-half. |
| 2026-05-17 | Must-fix #15-area — debt #15 DRAINED: post-create service attach (Agent 3 P1-3) **closed** + I1 fixed + D-6 discovered-deferred (supersedes the partial row above; left verbatim per append-only) | **closed** (P1-3 atomicity/data-loss class as written) | new gate `apps/api/scripts/smoke-attach-services.mjs` (44 probes) + `package.json`×2; `migration 00413_attach_services_invoker_align.sql` (I1 — verbatim repro of live 00412, sole delta `SECURITY DEFINER`→`SECURITY INVOKER`, on remote); `docs/smoke-gates.md`+`CLAUDE.md`+`docs/follow-ups/slice5-attach-services-decision.md`; see audit 03 ledger + Update 2026-05-17 (debt #15) | `pnpm smoke:attach-services` 44/0/0 exit 0 (run twice by orchestrator, pre-I1 + post-00413; gate honesty-audited line-by-line — probe 6 forces a real RPC-internal 23P01 + asserts zero partial rows); `tsc`+`errors:check-app-errors`(0/35)+`jest booking-bundles+reservations`(333/0) re-run by orchestrator; remote `prosecdef` attach=`f`==create=`f` post-00413; 00413 body diff-proven (sole line-4 delta vs live 00412) | The 2-agent full-review that f1085072 shipped without is now done (parallel fresh-context): code-reviewer clean; design-reviewer C1+I1. **C1 verified live-fireable vs ground truth → discovered finding D-6** (producer-determinism: `Date.now()`-derived `lead_time_remaining_hours` in the hashed `p_attach_plan` via `buildAttachPlan`; SOLE nondeterministic field; SHARED with the already-shipped create path so pre-existing, not slice-introduced) → deferred-with-owner, **bundled with debt #14/D-5** (same root class), tracked task #5 — NOT papered over. **I1 verified** (00412 `SECURITY DEFINER` vs template `SECURITY INVOKER`; false "IDENTICAL" doc claim) → fixed via 00413, doc corrected. P1-3 as written (kill the non-atomic TS `Cleanup`-queue data-loss class via an 00309-mirroring RPC) closed + smoke-proven; closes consistent with S1/S2 (closed + explicit deferred residual). |
| 2026-05-17 | Booking Slice 6 — P1-4 `BundleCascadeService.cancelLine`/`cancelBundle` TS-orchestrated → atomic RPC | **closed** | new `migration 00414_cancel_order_lines_with_cascade.sql` (on remote; ONE atomic tx replacing BOTH cancelLine + cancelBundle; mirrors cancel-family 00408 clause-for-clause; INTENT-hash idempotency; F-CRIT-1; durable `bundle.services_cancelled` outbox on the bundle path + new `bundle-services-cancelled-cascade.handler.ts`) + `bundle-cascade.service.ts` thin wrappers (dead choreography/`cancelBundleImpl`/`cancelOrdersForReservation`/`eventBus` removed) + 2 DELETE routes gain `RequireClientRequestIdGuard` + threaded actor; 6 error codes; new `smoke:cancel-order-line`; `docs/smoke-gates.md`+`CLAUDE.md`; see audit 03 ledger + Update 2026-05-17 (Slice 6) + `docs/follow-ups/slice6-cancel-order-line-plan.md` | `pnpm smoke:cancel-order-line` **55/0 exit 0** (orchestrator-run; probe 7 forces a real RPC-internal 22023 mid-rescope → 5 strict ZERO-partial-row assertions = atomicity proven; probe 8 REAL foreign-tenant booking → 404 + zero cross-tenant writes); `tsc` 0; `errors` 0/35; `jest booking-bundles+visitors` 408 pass (3 fail = pre-existing `visitors/admin.controller.spec.ts`, orchestrator-confirmed via stash-rerun = RLS-workstream artifact, NOT Slice-6); 00414 read line-by-line (#0-tenant on every write; 00408-faithful; deterministic) + remote `prosecdef=t` | Two-checkpoint review (codex 0-byte-hung→skipped per protocol). Plan-review checkpoint 1 caught 4 direction errors PRE-coding incl. a reviewer contradiction resolved against live code (per-line visitor cascade is a verified no-op — `adapter.ts:235` guard + `lineKindForOli` always 'other'). Impl-review + orchestrator smoke run caught + FIXED in-slice (not deferred): non-functional probe 7 (poison now genuinely raises), overstated probe 8 (real foreign-tenant fixture), null-actor (real `authUid` threaded — cancel-family-consistent). No CRITICAL. Accepted residual: `recurrence_scope` audit-shape drift (inert — recurrence-cancel routes via cancelOne→00408). |
...
| 2026-05-19 | **audit-03 deferred-closeout — COMPLETE** (cross-reference; canonical record = `03-booking-reservation.md` Closure Ledger + `### Deferred-closeout progress — 2026-05-19`) | **All 6 remaining audit-03 deferred items dispositioned: D-5/D-6/multi-slot/P2-3 closed+gated; D-9 closed; D-10/D-11/D-6-FE-pin/P2-3-workflow-def-atomic deferred-with-owner; D-12 not-raised (C2 fixed in-slice)** | Migrations 00430/00431/00432 live+verified on remote; reconciled onto origin/main (3db462e2/0d0e86d4/8167c37b). See `03-booking-reservation.md` 2026-05-19 ledger row + subsection. | Full live-smoke gauntlet GREEN: `smoke:edit-booking-scope` 49/0, `smoke:attach-services` p8, `smoke:recurrence-clone` 14/0, `smoke:create-multi-room` i/j/k, `smoke:outbox` 13/0, `smoke:edit-booking` FixtureE 12/12+FixtureD 11/11; tsc 0; errors 0; ZERO audit-03 regressions. | The booking-reservation specialist audit (03) is now fully closed best-in-class incl. its previously-deferred producer-determinism / create-path-consolidation slices. Per-item decision docs `docs/follow-ups/audit03-deferred-{d5,d6,multislot,p2-3}-decision.md`. |
| 2026-05-21 | **audit-02 final hardening** — FOLLOW-UP-A02-1 SLA cron reentrancy | **CLOSED (code + spec + docs)** | `SlaService.checkBreaches()` now has an in-process `checkBreachesInFlight` guard with `finally` release; overlapping minute ticks skip instead of racing the breach/at-risk/threshold pass. Docs synced in `assignments-routing-fulfillment.md`, `smoke-gates.md`, and audit-02 Closure Ledger. No migration, no production-visible debug trigger. | `pnpm -C packages/shared build`; `pnpm -C apps/api test src/modules/sla/sla.service.spec.ts` 13/13; `pnpm -C apps/api lint` green; `pnpm errors:check-app-errors` green. Smoke attempt: API build succeeded, but starting this worktree API on `PORT=3010` failed before listen because Supabase DNS could not resolve (`db.iwbqnyrvycqgnatratrk.supabase.co` `ENOTFOUND`; `nslookup` SERVFAIL). | This removes the bounded overlap race called out after the audit-02 re-review. The existing `smoke:tickets` SLA escalation probe remains the live real-cron gate for future SLA changes; no hook was added because unit tests can call the service method directly and a production trigger would add a second mutation surface. Rerun smoke once Supabase DNS is available. |
