...
| 2026-05-21 | **audit-02 final hardening** — FOLLOW-UP-A02-1 SLA cron reentrancy | **CLOSED (code + spec + docs + live smoke)** | `SlaService.checkBreaches()` now has an in-process `checkBreachesInFlight` guard with `finally` release; overlapping minute ticks skip instead of racing the breach/at-risk/threshold pass. Docs synced in `assignments-routing-fulfillment.md`, `smoke-gates.md`, and audit-02 Closure Ledger. No migration, no production-visible debug trigger. | `pnpm -C packages/shared build`; `pnpm -C apps/api test src/modules/sla/sla.service.spec.ts` 13/13; `pnpm -C apps/api lint` green; `pnpm errors:check-app-errors` green. Live Worker-backed API run on `:3001` from the audit worktree: `pnpm smoke:tickets` 123/0, including the real SLA escalation cron probe; `pnpm smoke:work-orders` 125/0. | This removes the bounded overlap race called out after the audit-02 re-review. The existing `smoke:tickets` SLA escalation probe remains the live real-cron gate for future SLA changes; no hook was added because unit tests can call the service method directly and a production trigger would add a second mutation surface. |
