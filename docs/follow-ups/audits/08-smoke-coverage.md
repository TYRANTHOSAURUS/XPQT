...
| 2026-05-17 | booking-audit Slice 6 — NEW `smoke:cancel-order-line` gate; closes the "per-line / bundle-services cancel has zero live coverage" gap (P1-4 regression boundary) | closed | new `apps/api/scripts/smoke-cancel-order-line.mjs` (55 assertions: per-line cancel atomic deltas; idempotency replay no-dup; payload-mismatch 409; fulfilled-line 422 protection; approval rescope (scope_breakdown shrink-not-close while other entities remain; expire when scope empties); bundle cancel both weak-close branches (pure-services booking → cancelled; kept/fulfilled line → booking stays) + `bundle.services_cancelled` outbox scoped via `payload->>'booking_id'`; **probe 7 load-bearing atomicity** — a `status='pending'` booking-targeted approval poisoned `scope_breakdown = '{"order_line_item_ids":"<scalar>"}'` forces a REAL RPC-internal `jsonb_array_elements_text(<scalar>)` 22023 mid-rescope (per-line route) → 5 strict assertions prove ZERO partial OLI/AR/WO rows + the `command_operations` in_progress row rolled back; **probe 8** — a REAL booking+OLI+AR+WO seeded under OTHER_TENANT, cancel as the real tenant → 404 + ZERO cross-tenant writes); registered `package.json`×2 + `docs/smoke-gates.md` + `CLAUDE.md` matrix | `pnpm smoke:cancel-order-line` **55/0 exit 0** (orchestrator-run). Backs `cancel_order_lines_with_cascade` (00414, cancel-family `prosecdef=t`). Probe 7 expects `error` (any non-2xx) because a forced raw-PG 22023 is an UNMAPPED error → correctly 500/`unknown.server_error`, NOT a user-actionable 422 (mirrors smoke-attach-services probe 6's "forced RPC-internal failure, prove rollback" rationale); the atomicity proof is the 5 strict zero-partial-row assertions, not the status. Per-booking-scoped (multi-session-safe; outbox via `payload->>'booking_id'`); skips NOT counted as pass; `exit 1` any fail / `exit 2` infra. See audit 03 ledger + Update 2026-05-17 (Slice 6). | Closes the per-line/bundle cancel coverage gap (the Slice-2 equivalence checklist explicitly deferred P1-4 out of `smoke:cancel-booking`). Probe 7 is a genuine bug-catcher: the FIRST run (pre-fix-cycle) FAILED 41/6 because the poison didn't actually raise (a non-functional gate the orchestrator's own run caught — the runnable-guards-mandate working); fixed so it forces a real 22023, then 55/0. NOT constructed-to-pass. Probe 8 first overstated coverage ("tenant-B header" the JWT-bound harness can't do) — strengthened to a real foreign-tenant booking fixture. |
| 2026-05-17 | booking-audit Slice 7 — NEW `smoke:recurrence-clone` gate; closes the "recurrence occurrence-clone has zero live coverage" gap (P2-1 boundary-retirement regression boundary) | closed | new `apps/api/scripts/smoke-recurrence-clone.mjs` (14 assertions: real `POST /api/reservations`+`recurrence_rule` daily×3 with services [mixed `repeats_with_series`] → master `confirmed` (self-diagnosing assertion — if `pending_approval` the gate fails LOUDLY with the off-hours-rule remediation hint, since `startSeries` is suppressed by design for pending bookings) → `recurrence_series` row created+anchored (bounded ≤40s poll, since `startSeries` is `void`-fired) → ≥2 occurrence bookings materialised → per-occurrence: catering[`repeats_with_series=true`] cloned, AV[`false`] NOT cloned, cloned order tagged `recurrence_series_id`, service window time-shifted onto the occurrence day not the master day, tenant-scoped #0; the forced-clone-failure→compensation branch is a PRINTED honest coverage boundary [NOT skip-as-pass, NOT a counted probe] — the only AR-conflict lever is caught at order.service.ts:275-281 before reaching compensation, so that path is covered by the 7 rewritten `recurrence-materialize.service.spec.ts` jest tests against the REAL `deleteOrphanOccurrence`+`delete_booking_with_guard` arg shape); registered `package.json`×2 + `docs/smoke-gates.md` + `CLAUDE.md` matrix | `pnpm smoke:recurrence-clone` **14/0 exit 0** (orchestrator-run). Asserts DB rows NOT HTTP status (`startSeries`/`materialize` are `void`-fired + `.catch()`-swallowed). Per-series/occurrence-scoped. `exit 1` any fail / `exit 2` infra. Backs the Slice-7 boundary retirement + the D-8 fix end-to-end. See audit 03 ledger + Update 2026-05-17 (Slice 7). | Genuine e2e regression catcher: it took **6 honest fix-cycle iterations**, each failure empirically root-caused by the orchestrator's own runs (NOT papered over): `client_line_id` 400 → `startSeries` void-fire race (poll) → fixture tripped the 00133 off-hours approval rule → `FIXTURE_DAYS` 90d-horizon (theory falsified by re-run) → **discovered pre-existing P1 D-8** (synthetic `system:*` actor → uuid bind 500; recurring bookings never materialised since 2026-04-25) proven via the dev-server log + fixed-in-slice. A boundary-removal OR D-8 regression would fail this gate (0 occurrences). The runnable-guards-mandate working: I ran it myself; a static reviewer called it "honest + a genuine catcher" but only RUNNING it surfaced the 6 real issues. |
| 2026-05-19 | audit-03 deferred-closeout — full live-smoke gauntlet GREEN on reconciled origin/main (cross-reference; canonical record = `03-booking-reservation.md` Closure Ledger + `### Deferred-closeout progress — 2026-05-19`) | closed | migrations 00430 (D-5 strip-helper) / 00431 (P2-3 combined-RPC 7→11-col approvals) / 00432 (revoke legacy 21-arg `create_booking`) live+verified on shared remote; reconciled onto origin/main 3db462e2/0d0e86d4/8167c37b | `smoke:edit-booking-scope` 49/0 (D-5 FIXME-409 flipped to cached-success — the authoritative D-5 gate) · `smoke:attach-services` probe 8 GREEN (D-6 lead-time-retry → cached-2xx) · `smoke:recurrence-clone` 14/0 (D-9 regression) · `smoke:create-multi-room` i/j/k GREEN (P2-3 no-services FLAT approval rows chain_id-bearing + 00402-inbox) · `smoke:outbox` 13/0 (P2-3 booking.created emit) · `smoke:edit-booking` FixtureE 12/12 + FixtureD 11/11 (multi-slot no-silent-corruption + single-slot regression); tsc 0; errors 0; ZERO audit-03 regressions | New live coverage vs the matrix: D-5 commit-replay now asserts cached-success (was a FIXME-409 honest-fail); D-6 lead-time-rule retry case now smoke-covered (`smoke:attach-services` p8); P2-3 no-services FLAT create approval + inbox now end-to-end (`smoke:create-multi-room` i/j/k); multi-slot deliberate-skip non-silent via the `booking.linked_rows_not_propagated` audit row (`smoke:edit-booking` Fixture E). Pre-existing-not-audit-03 probe-environment notes (create-multi-room a/h, attach-services 1, create-multi-room b replay-500) recorded in the 03 subsection. |
