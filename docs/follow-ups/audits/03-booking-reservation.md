...
| 2026-05-18 | D-5 (CLOSED — audit-03 Slice 2, producer-determinism) `edit_booking_scope` / `edit_booking` same-intent commit-RETRY → spurious `command_operations.payload_mismatch` 409 | closed | new `migration 00430_booking_edit_strip_hash_prestate_fields.sql` (verbatim re-create of the live `booking_edit_strip_hash_server_fields` from 00407:55-82; SOLE executable delta = exclusion set `('_resolution_at')`→`('_resolution_at','old_outcome','chain_config_changed')`; `language sql immutable` / `set search_path = public` / revoke+grant reproduced verbatim; wrapper + `edit_booking` + `edit_booking_scope` untouched — they call the helper by qualified name); `assemble-edit-plan.service.ts` (`shapeChainConfigForPlan` now `canonicalApproverSort`s `required_approvers` — closes a separate latent ≥2-approver order-instability); `assemble-edit-plan.idempotency.spec.ts` (new GUARD-3 completeness falsification + GUARD-2 reworked to assert the 3-name enumerated set + TS mirror extended); `docs/follow-ups/audit03-deferred-d5-decision.md` | jest GUARD-3 on HEAD: bug reproduced under `{_resolution_at}` (md5 differ); EXHAUSTIVE deep key-path diff of two post-`{_resolution_at}`-strip producer outputs = EXACTLY `{approval.old_outcome, approval.chain_config_changed}` for BOTH the no-chain→inserted-chain and chain→expired transitions (no third field — completeness-FALSIFIED, the prior-misdiagnosis trap closed); fix proven under the 3-name set (md5 identical). Post-fix `jest src/modules/reservations` + `tsc` + `errors:check-app-errors` clean. Live `smoke:edit-booking-scope` FIXME-409 block flipped to assert cached-success replay (authoritative completeness gate; validated in batch push). | TRUE root cause (verified, NOT re-derived): `old_outcome`/`chain_config_changed` are derived 100% from a live `approvals` read (`loadCurrentApprovalChain`) with zero caller input; the COMMIT's §3.6.5 reconciliation mutates `approvals`, the RETRY reads back mutated state → both flip → different post-strip md5. Pure PRE-STATE (the `_resolution_at` precedent class), fully re-derivable by the RPC — excluding from the HASH is semantically correct; the RPC still reads them from the UNSTRIPPED plan so §3.6.5 is unaffected. Forward-only on shared remote (rollback = follow-up migration; reverting reinstates the 409). Two prior misdiagnoses (array-canonicalization; one-line-hash-swap) explicitly recorded in the decision doc + the 2026-05-17 D-5 row above (history preserved, not rewritten). |
...
| 2026-05-17 | P2-2 `source='auto'` shim in TS (`:210`) | **closed** (Slice 8) | `dto/types.ts` `ReservationSource = BookingSource \| 'auto'` → `= BookingSource` ('auto' dropped); resolution HOISTED to producers: `recurrence.service.ts` emits `source:'recurrence'` directly (actor always `system:recurrence`), `multi-room-booking.service.ts` resolves inline (verbatim old rule: `system:recurrence`→'recurrence', other `system:*`→'calendar_sync', else `input.source??'portal'`); the two `'auto'` consumer-coercion blocks in `booking-flow.service.ts` (~:226/:874) collapsed to `input.source ?? 'portal'` (byte-identical non-'auto' default); `reservation.controller.ts` `'auto'` compat cast dropped; `booking-flow-build-attach-plan.spec.ts` 'auto' tests rewritten (assert producer→consumer pass-through, not gutted). NO migration; NO @prequest/shared (ReservationSource is reservations-local) | `tsc --noEmit` 0 (THE gate for type-only — a missed producer = compile error); `jest src/modules/reservations` 262/262; `errors:check-app-errors` 0/35; residual-`'auto'` sweep EMPTY (only unrelated `FulfillmentStrategy`/`TriggeredBy`/etc. unions remain) | 2-agent impl-review (codex 0-byte-hung→skipped): no CRITICAL/IMPORTANT. Make-or-break verified by both reviewers against live code: the ONLY non-recurrence `system:*` create caller (Outlook-sync, reservations.module.ts:184/200) passes `source:'calendar_sync'` EXPLICITLY and NEVER relied on the removed `'auto'`→calendar_sync consumer default; P3-1 (calendar-sync inbound CREATE deferred) re-verified true ⇒ NO silent source regression. Decision doc: `docs/follow-ups/slice8-p2-cleanup-plan.md`. |
| 2026-05-17 | P2-4 `recurrence_overridden` semantics live in two places (`:227`) | **closed** (Slice 8) | `edit-plan.types.ts` added `ScopeEditPlanBookingPatch = Omit<EditPlanBookingPatch,'recurrence_overridden'>` + `ScopeEditPlan` (Omit booking only); `assemble-edit-plan.service.ts` retyped `AssembleScopeEditPlanResult.rpc_plans[].plan` → `ScopeEditPlan` + a destructure-reconstruct projection (`const {recurrence_overridden:_x, ...scopeBooking}=plan.booking` + `void _x`) at the scope boundary so a scope plan CANNOT carry the field at the type level. Shared `buildSingleSlotPlan` + per-occurrence (`kind:'one'`/`'slot'`) path return type UNCHANGED. NO migration; NO runtime/payload change | `tsc --noEmit` 0; `jest src/modules/reservations` 262/262 (incl. the scope `recurrence_overridden:false` runtime backstop at `reservation-edit-scope.spec.ts:110` + `assemble-edit-plan.idempotency.spec.ts:236` — intact); `errors` 0/35 | Runtime byte-identical: the scope path's `auto_set_recurrence_overridden:false` means scope plans NEVER set the key at runtime, so the destructure-strip is a wire no-op. tsc-proof is at the assembler boundary; the pre-existing universal `reservation.service.ts:1801` `as unknown as` serialization cast is NOT a defeating re-cast (same pattern :1033/:1354); the `00395:218-222` RPC guard stays as last-mile defense-in-depth (unchanged). 2-agent impl-review: no CRITICAL/IMPORTANT. |
...
| 2026-05-18 | D-9 (closed, audit-03 Slice 1 — observability-only) — `RecurrenceService.materialize()` catch now reads `(err as AppError).code` via `isAppError`; dedicated `partial_failure` / `compensation_failed` triage logs fire AND a NEW dedicated `rule_deny` / `reservation_slot_conflict` triage log fires (`recurrence.service.ts:705-712`). **Net series-state effect byte-identical to pre-fix for EVERY branch**: the now-live `rule_deny`/`slot_conflict` branch ALSO sets `sawUnexpectedFailure=true` (recurrence.service.ts:709) so the gate at `recurrence.service.ts:725` (`!sawUnexpectedFailure`) does NOT advance `materialized_through` — exactly as the pre-fix catch-all did. The ONLY behavioural delta is which log line fires. | **closed** | `tsc` 0; `jest src/modules/reservations` — same 4 pre-existing `reservation-edit-scope.spec.ts` failures (count unchanged), rest green incl. the new `rule_deny`/no-advance assertion + the 2 existing triage-log assertions; `errors:check-app-errors` 0/35 | A correction was REQUIRED on the first D-9 commit (69eaddca): that commit let `rule_deny`/`slot_conflict` ADVANCE (it did `skip;continue` WITHOUT setting `sawUnexpectedFailure`), a behaviour change pre-fix did NOT produce. D-9 is scoped observability-ONLY; the advance-behaviour change is correctly deferred as **D-10** (below). No-advance now preserved byte-identically. |
| 2026-05-18 | D-10 (discovered while correcting D-9, audit-03 Slice 1 — pre-existing, NOT D-9-introduced; masked by the D-9 dead-branch bug) — `RecurrenceService.materialize()` never advances `materialized_through` past an occurrence whose create-time rule outcome is `rule_deny` / `reservation_slot_conflict` (`recurrence.service.ts:705-712`). These are user-correctable and will NOT unblock on a retry, so the series is effectively WEDGED at the first denied occurrence: no forward occurrence ever materialises until manual intervention. The old comment ("Skip + advance") asserted the opposite, but the dead branch never executed, so pre-fix every denied occurrence also fell to the catch-all → no advance. | **deferred-with-owner** (owner: a future booking-audit observability/recurrence slice) | n/a — explicitly NOT fixed here (the task author scoped D-9 observability-only; shipping the advance change unilaterally is the exact unverified-design-claim class this audit was burned by) | Risk: a single denied occurrence stalls ALL forward materialisation of that series until manual intervention; practical exposure depends on how often a recurrence master hits a denied room rule. **Fix design:** in the `rule_deny`/`slot_conflict` branch, skip-AND-advance (do NOT set `sawUnexpectedFailure`) — but that needs its OWN smoke proving forward occurrences still materialise and the denied one is correctly skipped. Precise site: `apps/api/src/modules/reservations/recurrence.service.ts:705-712` (the `if (code === 'rule_deny' || code === 'reservation_slot_conflict')` branch); gate it does NOT bypass: `:725`. |
| 2026-05-18 | P0-2 multi-slot residual (Slice-1 deferred row) — **CLOSED *as a silent gap* via Path B; the propagation gap itself → discovered finding D-11 deferred-with-owner** (audit-03 Slice 3). Supersedes the 2026-05-16 "P0-2 residual — multi-slot linked-row propagation gap | deferred" row (left verbatim per append-only). | **closed (silent-gap eliminated) + D-11 deferred-with-owner** | `edit-plan.types.ts` (SERVER-INTERNAL `EditPlan._skipped_multi_slot_linked_rows` — never sent to the RPC); `assemble-edit-plan.service.ts` (`buildSingleSlotPlan` now PROPAGATES `linked.skippedMultiSlot` — pre-fix it was DROPPED at that call site, making the skip SILENT below the `[I-1]` `log.warn`); `reservation.service.ts` (new `recordMultiSlotLinkedRowSkip` = durable best-effort tenant-scoped `audit_events` insert mirroring the in-service `booking.restored` pattern + `stripInternalMarkers` wire-strip; wired into `editOne`/`editSlot` [post-commit emit + strip] + `editScope` [strip-only — scope cannot time-shift ⇒ no daglijst divergence ⇒ no audit row, the strip keeps the marker off the scope RPC wire/hash]); `assemble-edit-plan.linked-rows.spec.ts` (NEW describe — drives REAL `buildSingleSlotPlan` for a >1-slot booking: marker set + 3 arrays empty; single-slot ⇒ marker absent; existing direct-helper multi-slot test UNCHANGED); `reservation-edit-slot.spec.ts` (NEW — marker STRIPPED off the RPC payload + durable tenant-scoped audit row with `{reason,edit_kind,slot_count,actor_user_id}`; negative no-over-fire test); `assemble-edit-plan.idempotency.spec.ts` (GUARD-2 EXEMPTION annotation, no assertion change); `apps/api/scripts/smoke-edit-booking.mjs` (NEW Fixture E = 2-slot booking + full linked-row graph + `runFixtureEProbe`; Fixture D regression UNCHANGED); `docs/smoke-gates.md`; NEW decision doc `docs/follow-ups/audit03-deferred-multislot-decision.md` | `tsc` 0; `jest src/modules/reservations` 4 failed / 268 passed — ALL 4 fails = the pre-existing `reservation-edit-scope.spec.ts` (B.4 Step 2F.3 splitSeries/editScope) defect, provably on pristine origin/main (this branch does NOT modify that spec; `git show origin/main:…reservation-edit-scope.spec.ts` already contains the failing `:1030` assertion with 0 conflict markers; the `recurrence.service.ts` 39+/5- diff vs origin/main is the parallel splitSeries-signature workstream, committed before this branch's HEAD, NOT this slice's change). Every new/edited spec in THIS slice passes; `errors:check-app-errors` 0/35. Live `smoke:edit-booking` Fixture E = the authoritative fail-closed gate (orchestrator runs it in the batch push pass). **NO migration · NO RPC change · NO response-wire change · NO success-path error-codes.ts entry.** | **Premise-falsified (the sharper truth):** the Slice-1 candidate "uniform-delta propagation for whole-booking moves" is FALSE today — `editOne` is itself under-defined for multi-slot (resolves/patches only the PRIMARY slot; v5 RPC writes `bookings.start_at/end_at` straight from the booking patch with NO MIN/MAX over `booking_slots`; the assembler defers the envelope MIN/MAX to a NON-EXISTENT "Step 2F"). So NO edit kind produces a uniform whole-booking move; the booking envelope goes inconsistent with its OWN slots BEFORE linked rows even matter. Generalizing propagation now would shift children to a window the other slots never moved to = WORSE than the skip. Path A (generalized propagation) is therefore an unspecced redesign of a CLOSED smoke-gated core path + an unresolved per-slot approval/cost/`recurrence_overridden` sub-decision ⇒ OUT of closeout scope; deferred-with-owner per the audit norm (P2-3 / D-5 / D-6 / D-10). `warnings[]` / success-path error code REJECTED (error-handling spec: closed RFC-9457 wire shape + closed surface enum + exhaustive 11 classes — never invent a surface). Mechanism chosen = (a) durable tenant-scoped `audit_events` row (no-migration TS path exists + is canon ⇒ (b) log-only fallback not needed). Decision doc: `docs/follow-ups/audit03-deferred-multislot-decision.md`. |
| 2026-05-18 | D-11 (discovered, audit-03 Slice 3 — pre-existing, NOT introduced here) — `editOne` HALF-MOVES a multi-room booking: it resolves/patches only the PRIMARY slot, and the v5 RPC writes `bookings.start_at/end_at` from the booking patch with NO MIN/MAX over `booking_slots` (the "Step 2F" envelope recompute is UNBUILT — `assemble-edit-plan.service.ts` ~:776-783/:839-840 comments), so a >1-slot `editOne` leaves the non-primary slot(s) at the old window AND the booking envelope inconsistent with its own slots — BEFORE linked rows matter. | **deferred-with-owner** (owner: a future booking-audit multi-slot-edit-semantics slice) | n/a — explicitly NOT fixed here (Path A is an unspecced redesign of a CLOSED smoke-gated core path — exactly the unverified-design-claim class this audit was burned by; closeout scope = defer-with-owner). Now MITIGATED, not silent: durable queryable `booking.linked_rows_not_propagated` audit row per occurrence + the Fixture-E safety smoke proves NO silent corruption (children + non-primary slot left exactly at seed, not shifted to a phantom window). | **Prerequisite:** define multi-room `editOne` semantics (block the edit vs. a TRUE whole-booking move with envelope MIN/MAX over ALL slots) AND resolve which slot drives `newOutcome` / approval / cost / `recurrence_overridden` when slots match different rules — only THEN is Path A (generalized linked-row propagation) well-defined. **Risk:** multi-room-with-services time edits leave the caterer daglijst / setup work_orders diverged from the moved primary slot until the semantics slice ships — mitigated (queryable signal + smoke proving no silent corruption), NOT papered over. Decision doc: `docs/follow-ups/audit03-deferred-multislot-decision.md` §5. |
| 2026-05-18 | P2-3 two `create_booking` RPC generations coexist (`:218`) — **CLOSED (flat case atomic) + DEFERRED-WITH-OWNER (workflow-def case engine-owned)** (audit-03 deferred-closeout). Supersedes the 2026-05-17 "deferred-with-rationale" P2-3 row (left verbatim per append-only — the prerequisite it named ["a future slice must FIRST relocate the pending_approval fan-out into a canonical no-services attach-plan path, THEN cut over + revoke"] is exactly what this slice did). | **closed (flat-atomic) + deferred-with-owner (workflow-def)** | **Migrations: 00431** (`create_booking_with_attach_plan` re-created VERBATIM from live 00372 — sole executable delta = step-10 approvals INSERT 7→11 cols; comment-stripped body diff proves ONLY +4 col names +4 value exprs +1 docstring; C1: `chain_threshold := coalesce(nullif(v_approval->>'chain_threshold',''),'all')`, the 3 nullable cols `nullif(...,'')::T`) + **00432** (`revoke execute on public.create_booking(<21-arg sig verbatim 00277:236-259>) from public,anon,authenticated,service_role` + `notify pgrst`; revoke-not-drop, forward-only). Code: `booking-flow.service.ts` (`create()` → unconditional `createWithAttachPlan`; legacy 300-line `create()` body + `createApprovalRows` DELETED; no-services FLAT approval builder in `buildAttachPlan` else-branch — `planUuid`-derived ids + shared `planUuid(key,'approval','__chain__')` chain id, `canonicalApproverSort` BEFORE mapping, person/team split = `createApprovalRows`, `any_pending_approval` + `bundle_audit_payload` in lockstep; WORKFLOW-DEF post-RPC `workflowService.start` best-effort try/catch; `buildAttachPlan` return extended with `approvalCutover`); `attach-plan.types.ts` (`AttachPlanApproval` +4 optional chain fields, `approver_person_id` → `string\|null`); `plan-sort.ts` (`comparePlanApprovals` null-safe). Specs: `booking-flow-atomicity.spec.ts` (no-services tests rewritten → assert combined RPC, legacy RPC NEVER called; default mock returns combined-RPC shape); `booking-flow-build-attach-plan.spec.ts` (NEW: FLAT person → approvals+chain+determinism across 2 same-key calls, threshold='any' → parallel null, TEAM → approver_team_id+person null, WORKFLOW-DEF → 0 plan rows + cutover def id, confirmed → empty). Smoke: `smoke-create-multi-room.mjs` NEW probes (h)-(k) + inbox/teams cleanup; `docs/smoke-gates.md` updated. | `tsc` 0; `jest src/modules/{reservations,booking-bundles,orders,routing}` — 4 failed / 497 passed / 1 skip / 15 todo; ALL 4 fails = the pre-existing `reservation-edit-scope.spec.ts` (B.4 Step 2F.3 splitSeries) defect (count UNCHANGED; this slice modifies NEITHER `reservation.service.ts` NOR that spec — verified via `git status`); every new/rewritten P2-3 spec passes; `errors:check-app-errors` 0/35. Live smokes NOT run here (orchestrator batch pass); `smoke-create-multi-room.mjs` `node --check`-verified; (h)-(k) are real fail-closed (`passAssertion`→`results.fail`→exit 1). | **Flat case is now ATOMIC + inbox-notified** (genuine improvement over the legacy best-effort post-RPC `createApprovalRows` that could fail silently → stuck booking). Workflow-def case is **equivalent — not improved, not regressed** — engine owns the approval rows; making engine-owned approvals atomic with the booking is out of P2-3 scope, deferred-with-owner = universal-workflow workstream (Phase 1.B+). Recurrence-determinism DECISION: KEEP the random SYSTEM_ACTOR idempotency key (contained by `existingIndices` skip — `attach_operations` is effectively single-shot for recurrence; threading a deterministic key would convert a self-healing path into a hard `payload_mismatch` on a between-tick rule edit). `booking.created`-for-no-services VERIFIED BENIGN (sole consumer `WorkflowSpawnWakeOnBookingCreatedHandler` is resume-only/needs a pre-existing link → no-op; re-analyse if a future consumer acts on no-services bookings). Decision doc: `docs/follow-ups/audit03-deferred-p2-3-decision.md`. |
| 2026-05-18 | C2 (discovered, audit-03 deferred-closeout P2-3 — SYSTEMIC, PRE-EXISTING, NOT P2-3-introduced) — `ApprovalRoutingService.assemblePlan` emitted with-services approval rows with NO chain columns, so the combined RPC's pre-P2-3 7-col INSERT persisted `approval_chain_id=NULL`; the 00402 inbox trigger `return new`-SKIPS on NULL chain_id ⇒ **EVERY with-services pending-approval booking created via the combined RPC has been silently un-notified** (present on `main` today; corroborated by the in-code admission at the old `booking-flow.service.ts:~592-598`). | **closed — FIXED IN-SLICE (default; D-12 NOT raised)** | `approval-routing.service.ts` (`assemblePlan` now derives ONE shared deterministic `approval_chain_id = planUuid(idempotencyKey,'approval','__chain__')` — NOT `randomUUID`, byte-stable across same-intent retries; `chain_threshold='all'` [the path aggregates per-line approvers with no single threshold concept — each is independently required ≡ all-of]; `parallel_group='parallel-'+target_entity_id`; `approver_team_id=null` [this path resolves ONLY person approvers — person/role→persons/derived→persons, never team]); `approval-routing.assemble-plan.spec.ts` (extended the deterministic-ids test: shared chain id byte-stable across 2 builds, chain_threshold='all', parallel_group, team_id null). | `tsc` 0; the extended `assemblePlan` determinism spec passes within the 497-passed run; `errors:check-app-errors` 0/35; live coverage: the existing `smoke:create-multi-room` (f) require_approval probe already asserts `approval_chain_id is not null` on with-services rows — now satisfied by this fix (it was a latent assertion the pre-fix code would have failed had it been reached with the combined RPC; the new (h)-(k) probes additionally cover the no-services FLAT path end-to-end incl. the inbox row). | `assemblePlan` HAD `idempotencyKey` + `target_entity_id` (deterministic planUuid bookingId) in scope ⇒ a deterministic shared chain id was derivable WITHOUT new infra ⇒ in-slice fix was sound + byte-safe ⇒ **D-12 NOT needed** (the brief's fallback). Existing with-services `assemblePlan` rows that previously omitted all 4 keys now carry them; the 00431 RPC's C1 `coalesce`/`nullif` binds keep the absent-key path byte-behaviour-identical so no pre-existing booking regresses. Decision doc: `docs/follow-ups/audit03-deferred-p2-3-decision.md` §3. |
| 2026-05-18 | D-6 (CLOSED — audit-03 deferred-closeout, producer-determinism) — `attach_services_to_existing_booking` (00412/00413) + `create_booking_with_attach_plan` (live 00372) bake wall-clock-derived lead-time values into the resolver OUTCOME serialized into the idempotency-hashed plans; a same-intent retry straddling a tenant lead-time `service_rules`/`room_booking_rules` boundary recomputes a different md5 → spurious `*.payload_mismatch` 409, op permanently lost. **SUPERSEDES the 2026-05-17 D-6 deferred row + slice5 decision §D-6** (append-only — those left verbatim). **3 FALSIFIED standing claims** (proven FALSE, see decision doc §1): (a) "`lead_time_remaining_hours` is the SOLE nondeterministic field" — there are THREE orthogonal vectors, one TIME-INDEPENDENT; (b) "create `p_booking_input` deterministic + collections pre-sorted" — NO_FIX create arm reproduces a 13-key-path diff; (c) "fix = persist basis in the `*_operations` row" — ARCHITECTURALLY DEAD: v6 (00302/00316) rolls back the `in_progress` row on the exact retry-triggering failures, so nothing server-persisted survives → basis MUST be request-canonical. | **closed** (one discovered residual deferred-with-owner: the FE-pin) | NO MIGRATION (RPCs hash whatever plan they receive; 100% TS producer/resolver/predicate-engine determinism). `dto/types.ts` (`ActorContext.resolution_basis_at`); `reservation.controller.ts` (`parseRequestTimeHeader` + default ONCE at `actorFromRequest`, honors `X-Request-Time`); `bundle.service.ts` (V1 — `hydrateLines(…, resolutionBasisMs)` replaces `Date.now()`; attach basis = `bookings.created_at` via `loadBooking` SELECT+`BookingRow.created_at`; `BuildAttachPlanArgs.booking.created_at`; canonical-sort `pending_setup_trigger_args.ruleIds`/`setup_emit.rule_ids`); `room-booking-rules/rule-resolver.service.ts` (V2 — `assembleContext` reads `scenario.resolution_basis_ms`; V3-order — `.order('id')` on `fetchCandidateRules`+`fetchAllRules`; basis onto ctx); `room-booking-rules/dto/index.ts` (`BookingScenario.resolution_basis_ms`); `predicate-engine.service.ts` (V3-time — `lead_minutes_lt`/`lead_minutes_gt` read `BaseEvaluationContext.resolution_basis_ms`); `service-catalog/service-rule-resolver.service.ts` (V3-order — `.order('id')` on `fetchAllRules`); `service-catalog/service-evaluation-context.ts` (thread basis → ctx); `orders/approval-routing.service.ts` (V3-order — `scope_breakdown.reasons` sorted by `(rule_id, denial_message)`); `reservations/booking-flow.service.ts` (create — `resolutionBasisAt`=`actor.resolution_basis_at`??now; threads to room resolver + `bundle.buildAttachPlan`; canonical-sort matched-rule-derived `policy_snapshot.*`/`applied_rule_ids` from ONE sorted array preserving effects/eval alignment); `reservations/multi-room-booking.service.ts` (same basis + sorted matched ids — inherits the producer); `docs/assignments-routing-fulfillment.md` (routing-doc sync — deterministic tie-break); new `bundle-attach-plan.determinism.spec.ts` (completeness guard); `bundle-build-attach-plan.spec.ts` (+`created_at` fixture); `apps/api/scripts/smoke-attach-services.mjs` probe (8) fail-closed determinism; `docs/smoke-gates.md`; new `docs/follow-ups/audit03-deferred-d6-decision.md` | jest completeness guard 5/5: NO_FIX attach md5(run1)≠md5(run2) (diff @ `pending_setup_trigger_args`+`scope_breakdown.reasons[2]`); NO_FIX create md5≠ (13-key-path diff @ `bi.status`/`policy_snapshot.*`/`applied_rule_ids`/`ap.*`); FIX attach `equal=true deepDiff=[]`; FIX create `equal=true deepDiff=[]` for BOTH `p_booking_input`+`p_attach_plan` — completeness `expect(diff).toEqual([])` PASSED ⇒ NO 4th vector. `tsc` 0; `jest src/modules/booking-bundles src/modules/reservations src/modules/room-booking-rules src/modules/routing` = 4 fail / 515 pass (the 4 = PRE-EXISTING `reservation-edit-scope.spec.ts` editScope fails, IDENTICAL on a clean `git stash` baseline — proven; zero routing/rule-resolver/booking-bundles regression from STEP 3); `errors:check-app-errors` 0. Live smokes deferred to orchestrator batch. | TRUE root cause: the resolver OUTCOME (rule fired vs not + matched-id order), NOT a raw `_`-prefixed field — a strip helper (the 00407/D-5 approach) does NOT apply. attach(server-only `created_at`) vs create(request-basis) ASYMMETRY is necessary (booking row exists for attach, not for create). STEP 3 V3-order tie-break is a STABILIZATION (equal specificity+priority → lowest-id-first, was DB-arbitrary), NOT a semantic redefinition; deny>approval>warn>allow + specificity/priority unchanged; no routing spec relied on the arbitrary order (verified — zero new fails). FE-pin = deferred-with-owner (decision doc §4): owner = future producer-determinism/FE-pin follow-up; risk = same-crid create AUTO-retry across a lead-time boundary recomputes a fresh server-default basis (attach immune — server `created_at`); mitigant (verified) = `apps/web/src/lib/query-client.ts:~29-31` sets mutations `retry:false` app-wide + create/attach hooks add no per-hook retry ⇒ same-crid auto-retry RARE ⇒ D-6 real-but-low-frequency; bounded residual OWNED, not silent; NO FE change shipped (the `X-Request-Time` controller seam is in place for the follow-up). Decision doc: `docs/follow-ups/audit03-deferred-d6-decision.md`. |
| 2026-05-18 | D-6 STEP-3 routing-doc sync — deterministic rule tie-break documented | closed | `docs/assignments-routing-fulfillment.md` (new note: among rules tied on BOTH specificity AND priority, the tie-break was DB-row-arbitrary [`Array.prototype.sort` is stable over the unordered fetch] and is now deterministic lowest-id-first via `.order('id')` on the room+service rule fetches) | rule-resolver / routing jest specs run in the D-6 batch: zero new failures (no existing routing expectation relied on the arbitrary order) | Mandated by CLAUDE.md (rule-resolver = routing code, same-PR doc sync). Stabilization only — the four routing axes + specificity/priority ordering + effect precedence are unchanged. |
| 2026-05-19 | audit-03 deferred-closeout — ALL remaining items dispositioned | closed (+gated) / deferred-with-owner | migrations 00430/00431/00432 live+verified on remote; reconciled onto origin/main (3db462e2/0d0e86d4/8167c37b); full live-smoke gauntlet GREEN (edit-booking-scope 49/0, attach-services p8, recurrence-clone 14/0, create-multi-room i/j/k, outbox 13/0, edit-booking FixtureE 12/12+FixtureD 11/11) | tsc 0; errors 0; base 0-behind; prefix-guard green; ZERO audit-03 regressions | D-5/D-6/multi-slot/P2-3 CLOSED+gated; D-9 CLOSED; D-10/D-11/D-6-FE-pin/P2-3-workflow-def-atomic deferred-with-owner; D-12 not-raised (C2 fixed in-slice); per-item decision docs audit03-deferred-{d5,d6,multislot,p2-3}-decision.md |
...
- Drift guard: GUARD-2 fails if a future `_`-prefixed `EditPlan` field is added without updating the SQL exclusion set; it does NOT catch a future non-`_`-prefixed request-varying field (documented residual). **Update — 2026-05-18 (audit-03 Slice 2, D-5):** the two non-`_`-prefixed PRE-STATE fields `approval.old_outcome` and `approval.chain_config_changed` are now EXPLICITLY ENUMERATED in the SQL exclusion set (migration `00430`) and in the TS mirror. GUARD-2 was reworked to assert the SQL set == the TS mirror == the exhaustive enumerated set `{_resolution_at, old_outcome, chain_config_changed}` (no longer just the `_`-prefixed subset), so GUARD-2 now COVERS these two and would fail on drift in either direction. A new GUARD-3 falsifies completeness (the post-`{_resolution_at}`-strip varying set is EXACTLY those two paths — verified by exhaustive deep key-path diff on the real producer path). The residual now narrows to: a FUTURE, currently-unknown non-`_` request-varying field NOT yet enumerated (the same general drift class, but the D-5 pair specifically is closed).
...
#### Update — 2026-05-17 (Slice 8 — P2-2/P2-4 closure + P2-3 deferred-with-rationale)

Original finding:
- `### P2-2. source='auto' shim in TS` (`:210`); `### P2-4. recurrence_overridden semantics live in two places` (`:227`); `### P2-3. Two RPC family generations of "create_booking" coexist` (`:218`)

Status:
- P2-2 closed; P2-4 closed; **P2-3 deferred-with-rationale** (audit P2-3 explicitly sanctions "OR document why it stays").

Changed:
- `apps/api/src/modules/reservations/dto/types.ts` (`ReservationSource` drops `'auto'`)
- `recurrence.service.ts` (emit `'recurrence'` directly), `multi-room-booking.service.ts` (inline resolution, verbatim old rule), `booking-flow.service.ts` (two `'auto'` consumer blocks collapsed), `reservation.controller.ts` (cast)
- `edit-plan.types.ts` (`ScopeEditPlanBookingPatch`/`ScopeEditPlan`), `assemble-edit-plan.service.ts` (scope-boundary `Omit<>` destructure-reconstruct projection + `rpc_plans` retype)
- `booking-flow-build-attach-plan.spec.ts` (the two `'auto'` tests rewritten — assert producer→consumer pass-through, not hollowed)
- NEW decision doc `docs/follow-ups/slice8-p2-cleanup-plan.md` (plan + plan-review remediation incl. the P2-3 C1 P0-regression finding)
- NO migration; NO `@prequest/shared`; P2-3 / `create_booking` / `createWithAttachPlan` / the `:372-383` fan-out UNTOUCHED (git-diff-verified by both reviewers).

Verified (orchestrator ran directly + both checkpoint-2 reviewers independently re-ran):
- `pnpm -s --filter @prequest/api exec tsc --noEmit` -> exit 0 (THE gate for type-only work — P2-2 exhaustiveness + the P2-4 narrow are tsc-enforced; a missed `'auto'` producer or a scope plan able to set `recurrence_overridden` is a compile error)
- `pnpm -s --filter @prequest/api exec jest src/modules/reservations` -> 262/262 (full working tree). The 4 `reservation-edit-scope.spec.ts` `splitSeries`-arity failures are PRE-EXISTING (code-reviewer stash-isolation-proven: they fail on pure HEAD with ALL Slice-8 changes stashed — the in-flight Slice-4-parallel splitSeries-signature fix, NOT Slice-8-caused, NOT owned here). Slice 8 introduces ZERO new failures.
- `pnpm -s errors:check-app-errors` -> 0/35
- residual-`'auto'` sweep EMPTY (every remaining `'auto'` is an unrelated union — `FulfillmentStrategy`/`TriggeredBy`/`FulfillmentShape`/routing — or a doc-comment)
- 2-agent plan-review (checkpoint 1) caught the P2-3 P0-regression PRE-coding (verified live). 2-agent impl-review (checkpoint 2) — no CRITICAL/IMPORTANT; both reviewers traced ALL `bookingFlow.create`/`createGroup` callers and confirmed the only non-recurrence `system:*` create caller (Outlook-sync) passes `source:'calendar_sync'` EXPLICITLY and never relied on the removed `'auto'` consumer default ⇒ P2-2 is semantically lossless. Codex 0-byte-hung all session → skipped per `feedback_review_loop_protocol`.

Remaining:
- **P2-3 deferred-with-owner** (see the P2-3 ledger row): legacy `create_booking` stays for the no-services single-room path — it is a single ATOMIC Postgres fn (NOT the data-loss class the RPC mandate targets); a safe cutover requires FIRST relocating the `booking-flow.service.ts:372-383` `pending_approval` workflow/`createApprovalRows` fan-out into a canonical no-services attach-plan path (the no-services `buildAttachPlan` hard-codes `approvals:[]`/`any_pending_approval:false`), else every approval-gated no-services create (incl. recurrence occurrences) silently persists `pending_approval` with zero approval rows — a P0. Owner: a future create-path-consolidation slice (relocate fan-out → cut over → `revoke … from public,anon,authenticated,service_role` [20-arg sig] → new confirmed+pending_approval no-services single-room smoke). Reasoned + ledgered, NOT papered over.
- P2-4 type-proof is at the assembler boundary; the pre-existing universal `reservation.service.ts:1801` `as unknown as` serialization cast (also :1033/:1354) is not a defeating re-cast; the `00395:218-222` RPC guard remains the last-mile runtime defense (unchanged) — accurately disclosed, not overclaimed.

#### Update — 2026-05-17 (Slice 9 — CLOSING: P3 disposition + deferred-findings register)

This is the closing slice (doc-only, NO code change — the audit's completion bar is "P3 documented/deferred; debts resolved or explicitly owned"). It records the final disposition of every P3 + every deferred discovered finding so the audit file alone is the complete record.

**P3 findings — current state re-verified by the orchestrator 2026-05-17:**

| P3 | Finding | Status | Owner / risk |
|---|---|---|---|
| P3-1 | Calendar/Graph inbound sync read-mostly, write-deferred (`room-mailbox.service.ts` — `BookingFlowService.create` not wired for inbound; "Phase C" TODO still present, re-verified) | **deferred-with-owner** | Owner: the MS-Graph / calendar-sync Phase-C workstream ([[reference_ms_graph_spec]]). Risk: NONE today — inbound create is intentionally not implemented (not a regression). `source='calendar_sync'` enum value already exists (00277) for that path. The Slice-8 P2-2 hoist preserved `'calendar_sync'` as a valid producer value (Outlook-sync passes it explicitly today for the detect-only path). |
| P3-2 | `bundle.service.ts` size | **deferred-with-owner (partially improved)** | Was 2339 lines at audit; **now 1989** (Slices 5/6 folded the attach + per-line/bundle cancel CHOREOGRAPHY into RPCs `00412`/`00414`, ~−350 lines). Remaining is plan-building + read-side projections + `editLine` + `onApprovalDecided` — NOT the data-loss choreography (that is gone). Owner: a future incremental `bundle.service` decomposition. Risk: maintainability only, ZERO correctness (the multi-write paths are now atomic RPCs). The audit's "~600 lines" prediction assumed a wholesale rewrite; the incremental RPC-fold delivered the correctness win without the full collapse — accepted. |
| P3-3 | `Reservation` type/naming residue | **deferred-with-owner** | Intentional caller-signature stability per the canonicalisation rewrite (slots are `booking_slots`; the TS surface still says `Reservation`). Owner: the global Phase-8 TS naming sweep (already tracked in `00-integrator-verdict.md` ledger as a separate cross-cutting item — 1053-line allowlist). Risk: cosmetic/none; does not affect correctness. |
| P3-4 | `bookings.source='auto'` mismatch | **CLOSED by Slice 8 P2-2** | The audit itself said "Already covered by P2-2." Slice 8 removed `'auto'` from `ReservationSource` + hoisted resolution to producers; the residual-`'auto'` sweep is EMPTY (no live producer/consumer). Closed. |

**Deferred discovered-findings register (consolidated — explicit owner + risk; the audit completion bar's "explicitly owned"):**

- **Debt #14 (D-5) + D-6 — producer-determinism (SAME root class, bundled).** `edit_booking_scope` (D-5) and `attach_services_to_existing_booking` (D-6) hash a producer-built plan; the producers emit a non-`_`-prefixed value that varies across a same-intent re-assembly — D-5: the scope producer in `assemble-edit-plan.service.ts`; D-6: `buildAttachPlan`/`hydrateLines` bakes a `Date.now()`-derived `lead_time_remaining_hours` into the resolver context (SHARED with the already-shipped create path, so pre-existing & latent there too) — defeating the idempotency md5 ⇒ a same-intent retry that straddles a rule/lead-time boundary spuriously raises `*.payload_mismatch` 409 and the op is permanently lost. **Owner: a dedicated producer-determinism slice.** **Fix design (both producers, once):** a stable per-idempotency-key time/resolution basis — capture the resolution timestamp (and any rule-eval inputs) on the FIRST attempt, persist it in the `command_operations`/`attach_operations` row, and reuse it on retry so the re-assembled plan is byte-stable (NOT a strip helper — the nondeterminism is in the rule OUTCOME, not a raw `_`-field). **Risk:** real but practical exposure LOW today (no production tenants; requires a tenant to author an active rule whose predicate references a wall-clock/lead-time field). It is genuinely OUT of the booking-audit's atomicity-canonicalisation scope (the RPCs ARE atomic; this is idempotency-determinism, a distinct class) — explicitly OWNED with the fix design recorded, NOT closed and NOT papered over. Decision docs: `slice4-split-recurrence-decision.md` R-e, `slice5-attach-services-decision.md` §D-6.
- **D-9 — `RecurrenceService.materialize()` `e.response?.code` dead catch-branches.** ~~deferred~~ **CLOSED 2026-05-18 (audit-03 Slice 1, observability-only).** Catch now reads `(err as AppError).code` via `isAppError`; the `partial_failure`/`compensation_failed` triage logs + a new `rule_deny`/`reservation_slot_conflict` triage log fire. Net series-state effect is **byte-identical to pre-fix for every branch** (the now-live `rule_deny`/`slot_conflict` branch ALSO sets `sawUnexpectedFailure=true` → no advance). The advance-behaviour question raised by the dead `"Skip + advance"` comment is NOT folded here — it is the separate, deferred **D-10** below. See the 2026-05-18 ledger rows.
- **D-10 — `RecurrenceService.materialize()` never advances past a `rule_deny`/`reservation_slot_conflict` occurrence (series wedges).** Pre-existing, NOT introduced by D-9 (it was MASKED by the D-9 dead-branch bug — pre-fix every denied occurrence fell to the catch-all → no advance, so the bug and the wedge were indistinguishable). `rule_deny`/`reservation_slot_conflict` are user-correctable create-time rule outcomes that will NOT unblock on retry, yet `materialize()` keeps `sawUnexpectedFailure=true` for them so the gate at `recurrence.service.ts:725` never advances `materialized_through` — the series effectively wedges at the first denied occurrence and no forward occurrence materialises until manual intervention. The original comment ("Skip + advance") claimed the opposite intent but the branch was dead, so the intended skip-and-advance never happened. **Owner: a future booking-audit observability/recurrence slice.** **Risk:** a single denied occurrence stalls ALL forward materialisation of that series until manual intervention; practical exposure depends on how often a recurrence master hits a denied room rule (a tenant with an active deny rule overlapping a recurring series). **Fix design:** in the `if (code === 'rule_deny' || code === 'reservation_slot_conflict')` branch (`apps/api/src/modules/reservations/recurrence.service.ts:705-712`), skip-AND-advance — do NOT set `sawUnexpectedFailure` — so the gate at `:725` advances past the (correctly-skipped) denied occurrence. Needs its OWN dedicated smoke proving forward occurrences still materialise AND the denied one is correctly skipped (NOT just a unit assertion — the wedge is a real-DB cron-loop behaviour). Explicitly OWNED with fix design recorded, NOT papered over and NOT shipped unilaterally inside an observability-only slice.
- **P2-3 — two `create_booking` generations coexist.** Deferred-with-rationale in Slice 8 (see the P2-3 ledger row + Update): legacy `create_booking` is a single ATOMIC Postgres fn (NOT the data-loss class the RPC mandate targets); a safe cutover must FIRST relocate the `booking-flow.service.ts:372-383` `pending_approval` workflow/`createApprovalRows` fan-out into a canonical no-services attach-plan path (else a P0 on every approval-gated no-services create incl. recurrence occurrences). **Owner: a future create-path-consolidation slice** (relocate fan-out → cut over → `revoke … from public,anon,authenticated,service_role` [20-arg sig] → new no-services single-room confirmed+pending_approval smoke).
- **P0-2 multi-slot residual** (Slice 1) + **Slice-2 I-1 dead-letter backstop** + **Slice-3 R-a/R-b/R-c** + **Slice-4 R-a..R-e** — each already deferred-with-owner in their respective ledger rows above; unchanged; no new actionable P0/P1/P2 hidden among them (re-confirmed).

**Honesty check (orchestrator self-audit before closing):** NO actionable P0/P1/P2 finding has been silently re-classified to P3 or buried. Every deferral above carries an explicit owner + risk + (for the producer-determinism class) a concrete fix design. P3-4 genuinely closed (sweep empty). P3-1/P3-2/P3-3 are genuinely lower-priority/cosmetic/not-implemented-by-design with zero correctness impact. The producer-determinism class is genuinely a distinct concern outside the atomicity-canonicalisation mandate, explicitly owned not dismissed.

#### Closing status — booking-reservation audit (2026-05-17)

- **P0-1, P0-2, P0-3** — CLOSED + smoke-gated (Slices 1/2; smoke:edit-booking, smoke:cancel-booking).
- **P1-1, P1-2, P1-3, P1-4, P1-5** — CLOSED + smoke-gated (Slices 2/3/4/5/6 + debt #15; smoke:create-multi-room, smoke:edit-booking-scope, smoke:attach-services, smoke:cancel-order-line).
- **P2-1** — CLOSED + smoke-gated (Slice 7; smoke:recurrence-clone).
- **P2-2, P2-4** — CLOSED (Slice 8; tsc-enforced type-only).
- **P2-3** — DEFERRED-with-rationale + owner (Slice 8 — a safe cutover requires prerequisite fan-out relocation; legacy fn is atomic+correct; consistency-not-correctness).
- **P3-1/P3-2/P3-3** — deferred-with-owner; **P3-4** — closed by P2-2.
- **Discovered findings:** D-1/D-2/D-4 closed (Slices 1/3); D-5/D-6 explicitly-owned (producer-determinism slice, bundled); D-8 closed-in-slice (Slice 7); D-9 **closed** (audit-03 Slice 1, observability-only, 2026-05-18 — net series-state byte-identical to pre-fix); D-10 **deferred-with-owner** (discovered while correcting D-9: `materialize()` wedges a series at the first `rule_deny`/`slot_conflict` occurrence; pre-existing, masked by the D-9 dead-branch bug; future observability/recurrence slice).
- **`BookingTransactionBoundary` family RETIRED** (Slice 7); the booking subsystem now has ONE pattern (one PL/pgSQL RPC per multi-write op) with `delete_booking_with_guard` as the in-RPC/direct compensation primitive.
...
### Deferred-closeout progress — 2026-05-19

Final close of the audit-03 **deferred-closeout** workstream (branch `audit-03-deferred-closeout`). All six remaining deferred items (D-5, D-6, the P0-2 multi-slot residual, D-9, P2-3, plus the in-slice-discovered C2) carried through the two-checkpoint review protocol (plan-review before coding + impl-review after), reconciled onto current `origin/main` (merge `3db462e2` + migration-prefix renumber `0d0e86d4` + fixture repair `8167c37b`), and proven by a full live-smoke gauntlet on the shared remote. **No append-only ledger row was rewritten; this subsection consolidates the per-item rows already in the Closure Ledger above (2026-05-18 D-5/D-9/D-10/P0-2-multislot/D-11/P2-3/C2/D-6 rows) into a single honest disposition.**

**Migrations LIVE + verified on shared remote `db.iwbqnyrvycqgnatratrk.supabase.co`:**

- **00430** — D-5 strip-helper: `booking_edit_strip_hash_server_fields` exclusion set extended `('_resolution_at')` → `('_resolution_at','old_outcome','chain_config_changed')` (sole executable delta vs the verbatim 00407 re-create).
- **00431** — P2-3 combined-RPC `create_booking_with_attach_plan`: step-10 approvals INSERT widened 7 → 11 cols; C1-safe binds `chain_threshold := coalesce(nullif(v_approval->>'chain_threshold',''),'all')` + the 3 nullable cols via `nullif(...,'')::T` (verbatim re-create of live 00372 otherwise).
- **00432** — revoke legacy 21-arg `create_booking` from `public,anon,authenticated,service_role` (revoke-not-drop, forward-only) → ACL now `postgres=X/postgres` only.
- D-6, the multi-slot residual, and D-9 are **producer-only / observability-only TS — no migration.**
- Migration prefixes were renumbered from `00428/00429/00430` (collided with origin's concurrent `00428_dispatch` / `00429_routing_decisions`) → `00430/00431/00432`; CI prefix-guard green; no numbers baked into TS comments.

**Live-smoke gauntlet (reconciled code, dev server on a free port, shared remote) — GREEN with hard exit-0 evidence:**

- `smoke:edit-booking-scope` **49/0** — the D-5 authoritative gate: the prior FIXME-409 probe is flipped to assert cached-success replay.
- `smoke:attach-services` probe **8 GREEN** — D-6: lead-time-retry now yields cached-2xx, not a spurious 409.
- `smoke:recurrence-clone` **14/0** — D-9 regression-clean (net series-state byte-identical to pre-fix).
- `smoke:create-multi-room` probes **i + j + k GREEN deterministic** — P2-3: no-services FLAT person+team approval rows now chain_id-bearing + 00402-inbox-notified; (k) covers recurrence-occurrence chain-col persistence via a real seeded `recurrence_series` parent.
- `smoke:outbox` **13/0** — P2-3: `booking.created`-for-no-services emit regression-clean.
- `smoke:edit-booking` **Fixture E 12/12 + Fixture D 11/11 GREEN deterministic** — Slice-3 multi-slot no-silent-corruption + durable `booking.linked_rows_not_propagated` audit row; single-slot propagation regression intact.

**ZERO audit-03 regressions across the gauntlet.**

**Per-slice disposition:**

- **D-9 — CLOSED** (observability-only, byte-identical no-advance; jest-proven + `smoke:recurrence-clone` regression GREEN). The advance-behaviour question it surfaced is the separate **D-10** below.
- **D-5 — CLOSED + gated.** Root cause corrected on the third pass — the varying fields are the pre-state approval reads `old_outcome` / `chain_config_changed` (derived 100% from a live `approvals` read), **NOT** the digest's `required_approvers` (two prior misdiagnoses recorded in the decision doc + the 2026-05-17 D-5 ledger row, history preserved). Completeness falsified via GUARD-3; migration 00430; `smoke:edit-booking-scope` flip GREEN live.
- **Multi-slot (P0-2 residual) — CLOSED via Path B.** The Slice-1 "uniform-delta propagation" premise is FALSE today (`editOne` is not a whole-booking move for multi-slot — see D-11). A durable best-effort tenant-scoped `booking.linked_rows_not_propagated` audit event makes the deliberate skip non-silent; `warnings[]` / a success-path error code were rejected as spec-illegal (closed RFC-9457 wire shape + exhaustive 11 error classes). Fixture E live-GREEN proves no silent corruption.
- **D-6 — CLOSED.** Plan-review BLOCKED the naive Shape-C fix → 3 nondeterminism vectors fixed: V1 `hydrateLines`, V2 `assembleContext`, V3-time predicate-engine `lead_minutes_*`, V3-order deterministic rule `.order('id')`. attach basis = server-only `bookings.created_at`; create basis = request-canonical `ActorContext.resolution_basis_at` (asymmetry is necessary — the booking row exists for attach, not for create). Completeness-falsified (no 4th vector); routing-doc §25 synced; `smoke:attach-services` probe 8 GREEN.
- **P2-3 — CLOSED (flat-atomic).** No-services single-room consolidated onto the combined RPC; the C1 NOT-NULL trap was caught pre-coding and fixed; **C2 fixed in-slice** (the with-services `assemblePlan` now emits a deterministic shared `planUuid` `approval_chain_id` — previously-silently-un-notified with-services approvals are now 00402-inbox-notified; **D-12 NOT raised** — the brief's fallback was unnecessary). Legacy `create_booking` revoked (00432). Determinism-gated (planUuid + canonicalApproverSort; no `randomUUID` / `Date.now()` in hashed approvals); probes i/j/k GREEN live.

**Deferred-with-owner (explicit, not papered over):**

- **D-10** — `materialize()` wedges series forward-materialisation at the first `rule_deny` / `slot_conflict` occurrence. Pre-existing; owner = future observability/recurrence slice; revisit-trigger = a customer hitting a denied room rule on a recurrence master. Fix design + precise site recorded in the 2026-05-18 D-10 ledger row.
- **D-11** — `editOne` semantically under-defined for multi-slot (half-moves a multi-room booking; envelope MIN/MAX recompute "Step 2F" is unbuilt). Owner = future multi-slot-edit-semantics slice; prerequisite = define multi-room `editOne` semantics + per-slot approval/cost/`recurrence_overridden`; risk mitigated now via the queryable `booking.linked_rows_not_propagated` audit event + Fixture-E proof of no silent corruption.
- **D-6 FE-pin** — full create-path determinism needs the booking-composer to mint + resend `requested_at`. Mitigated by app-wide `retry:false` (`apps/web/src/lib/query-client.ts`) making same-crid auto-retry rare → D-6 real-but-low-frequency; owner = future producer-determinism/FE-pin follow-up; the `X-Request-Time` controller seam is in place. No FE change shipped.
- **P2-3 workflow-def-atomic** — the workflow-def no-services create keeps a post-RPC `workflowService.start` hybrid (engine owns those approval rows). Equivalent-not-regressed vs the deleted legacy path; full atomicity = a future workflow_instances-aware-RPC slice (universal-workflow workstream, Phase 1.B+).

**Pre-existing, NOT audit-03 (documented, flagged to owners, NOT fixed here):**

- The 4 `reservation-edit-scope.spec.ts` B.4-Step-2F.3 splitSeries jest failures — committed on `origin/main` by a parallel splitSeries-signature workstream; audit-03 touches neither that spec nor its source; count unchanged across the entire closeout.
- `smoke:create-multi-room` probes (a)/(h) + `smoke:attach-services` probe (1) fail on the shared remote due to a tenant-wide baseline `room_booking_rules` (`b001000{1,2,3}`, created 2026-04-25, 24 days pre-audit-03) — a probe environmental-assumption issue; P2-3's deliverable is proven by i/j/k.
- `smoke:create-multi-room` probe (b) idempotency-replay-500 = the documented pre-existing booking-create-via-combined-RPC replay bug-class (handoff §1; distinct from D-5's edit-path fix; out of audit-03 scope).
- A `smoke:edit-booking` Fixture-B editSlot idempotency-replay audit-delta quirk (pre-existing, not audit-03-authored).

**P3 — UNCHANGED.** No new P3 work this closeout (scoped as "only if time remains"). The prior Slice-9 disposition stands: P3-1 (calendar-sync Phase-C) / P3-2 (`bundle.service.ts` size) / P3-3 (naming residue) deferred-with-owner; P3-4 closed-by-P2-2.

**Reconcile / environment notes (for the audit record):**

- Reconciled onto current `origin/main`: merge `3db462e2`, migration-prefix renumber `0d0e86d4`, fixture repair `8167c37b`. Base 0-behind `origin/main`; CI prefix-guard green.
- A concurrent session left an uncommitted `git stash apply` contaminating 5 files mid-session — user-authorized surgical `git restore` cleanup; 17 stashes preserved; not audit-03's.
- The 2026-05-18 Codex Deep Review Verdict edit had been an uncommitted edit on the stale prior branch, dropped on the clean-base rebase; it was restored verbatim earlier this session.
- Commits carry an auto-configured `GOJIRA <x@user.local>` committer identity — git config was NOT modified (safety protocol); flagged for the user.
- Per-item decision docs: `docs/follow-ups/audit03-deferred-d5-decision.md`, `audit03-deferred-d6-decision.md`, `audit03-deferred-multislot-decision.md`, `audit03-deferred-p2-3-decision.md`.
