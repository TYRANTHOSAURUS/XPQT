...
- **D-9 — `RecurrenceService.materialize()` `e.response?.code` dead catch-branches.** `AppError` has no `.response`, so the `code === 'booking.partial_failure'`/`'booking.compensation_failed'` branches never fire; the catch-all preserves the don't-advance-`materialized_through` invariant **byte-identically (correctness UNAFFECTED — orchestrator-verified V1)**; only the dedicated ops-triage log lines are missed (ops sees a generic "unexpected (will retry)"). Pre-existing, NOT Slice-7-caused. **Owner: a future booking-audit observability cleanup.** Risk: observability-only. Fix = read `(err as AppError).code`.
...
- **Discovered findings:** D-1/D-2/D-4 closed (Slices 1/3); D-5/D-6 explicitly-owned (producer-determinism slice, bundled); D-8 closed-in-slice (Slice 7); D-9 deferred-with-owner (observability).
