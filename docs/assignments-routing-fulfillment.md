...
**Recurrence-safety contract (audit-02 P0-2).** The assignment RPC is the load-bearing, atomic, audited write. Everything after it in `fireThreshold` — the **watcher copy** and the **notification** — is *best-effort*: on failure each emits telemetry (`sla_escalation_watcher_skipped` / `sla_escalation_notify_failed`) and execution continues to write the **crossing row**, which is the idempotency anchor `selectApplicableThresholds` uses to suppress re-fire. This is deliberate: if a post-assignment side-effect threw, the assignment would already be committed but no crossing would be recorded, so the cron would re-fire the same escalation every tick forever (the assignment RPC would replay harmlessly, but the failing side-effect would re-throw indefinitely). Best-effort side-effects + always-anchored crossing make the escalation recurrence-safe without distributed transactions. Only a failure of the `writeCrossing` insert itself leaves a bounded retry window (RPCs idempotent on replay).
