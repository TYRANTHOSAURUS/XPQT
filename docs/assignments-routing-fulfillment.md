...
- `ApprovalService.respondToApproval` dispatches to `BundleService.onApprovalDecided` once the bundle's approval is fully resolved. Resolution rules:
  ...
  - Otherwise (the common bundle case — `ApprovalRoutingService` upserts one row per unique approver with `parallel_group=null`/`chain=null`): wait for **every** approval row on the same `target_entity_id` to be resolved (`approved` or `expired`; `pending`/`rejected` block). Treating "no group, no chain" as "fire on this grant" would re-fire for the first approver while peers are still pending — `areAllTargetApprovalsApproved` is the gate that prevents that. `expired` rows count as resolved because `BundleCascadeService.rescopeApprovalsAfterLineCancel` expires approvals whose scope is emptied by a line cancel; treating `expired` as blocking would deadlock the bundle.
