...
- **Assignment** → `set_entity_assignment` (00327 v2) with idempotency key `sla:escalation:<sla_timer_id>:<at_percent>:<timer_type>`. The key components are exactly the established crossing identity `crossingKey` = `sla_timer_id | at_percent | timer_type` (`sla-threshold.types.ts:33`): a single timer crosses 80% then 100% (`at_percent`), and a `both`-scope threshold crosses for the **response** and **resolution** timers at the same `at_percent` (`timer_type`) — each is its own idempotent canonical event. Dropping `timer_type` would collide the two legitimately-distinct `both`-scope crossings onto one `command_operations` key. A re-fired cron tick for the **same** crossing replays the cached result instead of re-applying. `reason` is non-null (`"SLA escalation — <at_percent>% threshold breached"`) so the RPC writes a `routing_decisions` row (`strategy='manual'`, `chosen_by='manual_reassign'`), a `reassigned` `ticket_activities` row, and a `ticket_assigned` `domain_events` row in one transaction. Actor is the system/cron (null). Entity kind (`case` vs `work_order`) is resolved **once** by `loadTicketForFire` — the same query that loads the row for the notification copy — and threaded through; there is no separate existence probe.
- **Watchers** → `update_entity_combined` (00384 v6) metadata branch, key `sla:escalation:<sla_timer_id>:<at_percent>:<timer_type>:watchers`, called only when the watcher set actually changes. The previous assignee is followed onto the ticket as a watcher. `tickets.watchers` is a **persons.id[]** column, so the outgoing assignee's `users.id` is translated to its `persons.id` before being added (the legacy raw path added the `users.id` directly — a latent person/user-ID-mixing bug now corrected on this path).
...
**Cron reentrancy guard (audit-02 follow-up, 2026-05-21).** `SlaService.checkBreaches` is class-level guarded: if a prior minute tick is still running, the next tick returns without starting the breach / at-risk / threshold passes. The guard releases in `finally`, so a thrown tick does not suppress future checks. This is an extra belt on top of the crossing unique key and `command_operations`; it removes the overlapping-cron race from the normal NestJS scheduler path without adding a production debug endpoint or RPC that could mutate SLA state outside the scheduled job.
...
- `RoutingEvaluationHandler` — drains `routing.evaluation_required`. Calls `RoutingService.evaluate`, then **always** calls `set_entity_assignment` (v3, supersedes 00327) with `idempotency_key=routing-evaluation:<event_id>` on every non-failure path. The `tickets.routing_status='idle'` / `routing_failure_reason=null` clear is **folded into that RPC's single transaction** via the v3 opt-in `clear_routing_status: true` payload flag (audit-02 P1-2, 2026-05-17) — atomic with the assignment; the former separate post-RPC raw `tickets.update` is gone, so a crash can no longer leave the ticket assigned with `routing_status` pinned at `'pending'`. Payload shapes: target found AND differs from current → include all three `assigned_*_id` keys (RPC applies the new assignment); unassigned outcome OR target already matches current → **omit** the assignee keys (key-absent = "no change" so a standing assignee is preserved — an `unassigned` result must not wipe assignment per v5/I4) — the RPC's v3 no-op fast path is **skipped** because `clear_routing_status` is set, so the `routing_status` clear still runs atomically. The handler still inserts a `routing_decisions` audit row (including `unassigned` outcomes per v5/I4) with `entity_kind='case'` + `case_id` set **explicitly** at the insert site (audit-02 P2-2 tail closed for this site — no longer relies on the 00232 derive trigger), in both the success path and the `markRoutingFailure` breadcrumb. **Idempotent under outbox redelivery (audit-02 Code-I1, 2026-05-18):** both of these handler-written `routing_decisions` inserts (every row carries `context.outbox_event_id = event.id`) are now raw parameterised writes with `ON CONFLICT (tenant_id, (context->>'outbox_event_id'), chosen_by) WHERE context ? 'outbox_event_id' DO NOTHING`, backed by the partial UNIQUE index `uq_routing_decisions_outbox_event` (migration `00429`). An outbox redelivery / `sweepStaleClaims` re-claim / multi-replica double-drain of the same event no longer writes a duplicate decision row — it is conflict-skipped (§6 success: idempotent success, no throw; `markRoutingFailure`: silent success, still WARN-only). The §6 success path still THROWS on a *genuine* DB error (the outbox retry contract is intact); only the conflict-skip is swallowed. `routing_decisions` rows written by the manual-reassign / SLA-escalation / `set_entity_assignment`-internal paths carry **no** `outbox_event_id`, fall outside the partial index, and remain unconstrained append-only (their idempotency is the upstream `command_operations` key, not this index). **Forward-only coupling:** the handler's explicit `ON CONFLICT` target hard-requires the 00429 index — dropping `uq_routing_decisions_outbox_event` while the handler change is deployed makes every routing eval throw `audit_insert_failed`; deploy index-first, never drop the index independently. (A separate, **pre-existing + orthogonal** `set_entity_assignment` idempotency-key/payload-stability defect on an assignment-changing routing-eval *retry* — distinct from this `routing_decisions`-insert idempotency — is routed as **I3** in `docs/follow-ups/i3-routing-eval-assignment-rpc-payload-drift-2026-05-18.md`; it is NOT a Code-I1 regression.) `'failed'` is still set out-of-band on resolver throws or RPC errors via `markRoutingFailure` (raw `tickets.update` + `routing_evaluation_failed` activity breadcrumb) — those paths skip the RPC. **Case-only by construction:** the only `routing.evaluation_required` producers are the `reclassify_ticket` RPC (00354) and the `grant_ticket_approval` RPC (00358), both emitting `aggregate_type='ticket'` with a `ticket_id`; there is no work_order producer, so the `p_entity_kind:'case'` hardcode is correct and a WO id would fail loud as `set_entity_assignment.not_found`. WO routing-evaluation is an explicit **future gap** (a future WO producer would need its own event + handler branch; `set_entity_assignment` v3 also raises `set_entity_assignment.routing_status_unsupported_for_work_order` if `work_order` + `clear_routing_status` is ever passed, since `work_orders` has no `routing_status`/`routing_failure_reason` columns).
