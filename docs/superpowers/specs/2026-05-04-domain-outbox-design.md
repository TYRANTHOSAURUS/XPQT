# Domain Outbox Design Specification — Plan B.1 (v8.1)

> **Authored:** 2026-05-04
> **Phase:** 6 (Durable Infrastructure)
> **Scope:** Investigation + Design only. No implementation code beyond this spec.
> **Status:** v8.1 is a tight mechanical patch on top of v8 that folds the codex v8 review (1 critical + 1 important + 1 nit). v8.1 is the FINAL design round before B.0 implementation begins. After v8.1 ships, B.0 starts immediately.
...
begin
  ...
  -- requester_person_id MUST be NULL for setup WOs (v8.1 — folds codex-v8 C1).
...
comment on function public.validate_setup_wo_fks(uuid, uuid, jsonb) is
  'Validates every tenant-owned FK in a setup WO row payload before INSERT. Folds v8-C1 of the outbox spec — closes the hole where create_setup_work_order_from_event trusted FK fields from p_wo_row_data without tenant validation. v8.1: also rejects non-null requester_person_id to preserve the legacy visibility contract (setup WOs are internal-only).';
```
...
- **v8.1: setup-WO RPC rejects non-null `requester_person_id`** — call `create_setup_work_order_from_event` with a `p_wo_row_data` payload whose `requester_person_id` is set (use a valid same-tenant person UUID and, in a sibling test, a cross-tenant person UUID). Assert the RPC raises `setup_wo.requester_person_id_not_allowed` with structured error before any insert; assert NO `work_orders` row, NO `setup_work_order_emissions` row. This guards the legacy visibility contract at `ticket.service.ts:1889`.
...
- ~~v6 I2 (`X-Client-Request-Id` mechanism referenced but unimplemented)~~ — initially v7 added `apiFetch` auto-stamping; superseded by v8-I1 / v8.1 which moves id generation to mutation-attempt scope inside producer hooks (the v7 auto-stamp was unstable across React Query retries). Middleware still exposes `req.clientRequestId`; producers thread the attempt-scoped id as idempotency_key, §3.3.
...
- `apps/web/src/lib/api.ts` — `apiFetch` forwards `X-Client-Request-Id` only when the caller supplies it via `options.headers`; the v7 auto-stamp is **dropped** in v8.1. The id is generated at mutation-attempt scope inside producer hooks (e.g. `useCreateBooking`, `useGrantBookingApproval`) — captured before `mutate()` — so React Query retries of the same attempt reuse it. Backend route guard requires the header on booking/approval producer routes. **v8.1 modification** (§3.3, supersedes the v7 auto-stamp).
...
- apiFetch: `apps/web/src/lib/api.ts:113-165` (v7 added an `X-Client-Request-Id` auto-stamp; **v8.1 drops the auto-stamp** and moves id generation to mutation-attempt scope inside producer hooks — see §3.3).
...
### 15.7 `X-Client-Request-Id` middleware + apiFetch tests (NEW in v7; rewritten in v8.1)

Per v8 §3.3, `apiFetch` does NOT auto-generate `X-Client-Request-Id`. Producer mutation hooks generate the id once per attempt and thread it as a header so React Query retries of the same attempt reuse it. Backend producer routes require the header.
...
- **Missing header (non-producer route) → server default**: assert `req.clientRequestId` is a fresh UUID and `req.clientRequestIdSource = 'server_default'`.
...
- **Case-insensitive header**: `x-client-request-id: <uuid>` (lowercase) is accepted.
- **Producer route requires header**: booking/approval producer routes with no `X-Client-Request-Id` return 400 with a structured error; no business RPC runs.

Frontend `apiFetch` tests (no auto-stamp):
- **`apiFetch` does NOT add `X-Client-Request-Id`**: monkey-patch `fetch`, call `apiFetch('/foo', { method: 'POST' })` with no caller-supplied header; assert no `X-Client-Request-Id` is sent.
- **Caller-supplied header preserved verbatim**: `apiFetch('/foo', { method: 'POST', headers: { 'X-Client-Request-Id': '<fixed-uuid>' } })`; assert the fixed value is sent unchanged.

Producer mutation hook tests (mutation-attempt scope):
- **Hook generates id ONCE per mutation attempt**: render `useCreateBooking`, trigger `mutate()` once with a `crypto.randomUUID()` mock that increments per call, configure React Query to retry N=3 times by failing the first N-1 fetches; assert the captured `X-Client-Request-Id` is **identical** across all N fetch calls (one mock-id consumed, not N). This is the regression that v7's fetch-scope auto-stamp had.
- **Different attempts get different ids**: two successive `mutate()` calls produce two different header values.
- **Hook threads id to backend route**: the request hitting the producer route includes the attempt-scoped id.
...
13. **v8.1: `apiFetch` + producer-hook unit-tested** (supersedes v7's auto-stamp checklist): `apiFetch` does not add `X-Client-Request-Id` when the caller doesn't provide one; caller-supplied header is preserved verbatim; producer hooks (`useCreateBooking`, `useGrantBookingApproval`) generate the id once per `mutate()` attempt and the same value is sent on every React Query retry of that attempt. See §15.7.
...
- v8.1 — 2026-05-04. Status: DESIGN — **FINAL design round before B.0 implementation begins.** Mechanical patch on top of v8 (commit `490c359`). Folds codex v8 review: C1 + I1 + Nit. No architecture changes. After v8.1, B.0 implementation starts immediately.
- v8 — 2026-05-04. Status: superseded (commit `490c359`). Folded 1 critical + 6 importants + 1 nit from codex v7 review, plus an explicit "Not in B.0" deferral section.
