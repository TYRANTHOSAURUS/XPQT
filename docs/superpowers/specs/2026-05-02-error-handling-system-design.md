...
| 8 | Form-draft preservation on auth-redirect — `sessionStorage` or in-memory? | `sessionStorage` so it survives the actual sign-in redirect (which is a full page nav). Cleared on rehydrate. User-scoped key per §6.2 GDPR rule. |
| 9 | Do we ship `messages.fr.ts` (Wallonia is in primary market) in v1, or defer? | **Defer.** v1 ships `en` + `nl`. **Trigger to ship `fr-BE`:** the first BE-French tenant signs OR before the first sales demo to a Wallonian prospect — whichever is earlier. Owner: whoever owns the sales pipeline at that point. The registry already supports `messages.<locale>.ts` arbitrarily; adding `fr` is a translation-effort question, not an architecture one. |
| 10 | How does the error system handle server-side async integration validation (SMTP test, SSO metadata fetch, webhook ping)? | **Out of scope for v1.** These don't fit `validation` (no Zod `fields[]`) and shouldn't toast as `server` (the cause is configuration, not a bug). Today they're handled inline by each integration's own admin page (e.g. `/admin/integrations/email` shows test results inline). The error system's wire shape supports these as `integration.test_failed` codes if/when needed; defer until at least three integration surfaces have shipped a "test before save" UX. **Trigger to revisit:** when the third integration admin surface ships. |
| 11 | Field-tech / vendor / mobile mutation idempotency on flaky networks (work-order updates that may double-apply) — handle in this spec or separately? | **Out of scope for v1; tracked as a follow-on contract change.** Idempotency is a wire-shape concern (`Idempotency-Key` request header + server-side dedupe) more than an error-UX one. Add the header convention to the wire-shape spec when the field-tech surface ships its first non-idempotent mutation. **Trigger to ship:** before the field-tech iPad surface goes to a real customer pilot (project memory says this is upcoming). |
| 12 | Mobile-specific rules (toast position with iOS keyboard up, scroll-to-error with keyboard occluding bottom 40%, banner height on 667px viewport)? | **Defer to a mobile-portal-specific spec.** The portal is mobile-first; the desk/admin app is desktop-primary. Each surface needs its own keyboard-aware rules; trying to spec them in one place inflates this spec. **Trigger:** when the mobile portal team picks up the rendering layer; they own the mobile rules. |
