...
| 8 | **Every error has a recovery.** If the design can't name one, the class is wrong. Recovery options are typed: `retry` · `wait` (timed-retry, distinct from `retry` because the action is "do nothing") · `signIn` · `reload` · `goBack` · `pickAlternative` · `askAdmin` · `requestAccess` (self-serve: open a ticket against the relevant admin role) · `contactSupport` · `copyDraft` · `dismiss`. | "Try again" is the floor, not the ceiling. The button labels are the UX. `wait` is distinct so the renderer can show a disabled-with-countdown button rather than a Retry that's wrong-labeled. `requestAccess` is distinct from `askAdmin` because it's an action, not a name to look up — the user clicks and an access-request ticket is created. `copyDraft` is the v1 conflict-toast partner action (see §6.3). |
...
export type Recovery =
  ...
  | { kind: 'requestAccess'; permission: string; resourceId?: string; submit: () => void }  // creates an access-request ticket
