...
**Status:** v4 DRAFT — Checkpoint-1 plan-review (round 4). No code yet.
...
- **Migration slots have shifted again.** Between v3 plan-review and v4, 00381 was taken by `00381_planning_smoke_requester_seed.sql`. **v4 renumbers: 00382 = schema + backfill + tenant triggers + ensure-RPC + cancel-with-approvals RPC, 00383 = grant_booking_approval v2 supersession.** All references in §3, §6, §10, citation index updated. Latest-prefix discipline maintained (no 00375 backfill).
...
3. **Per-rule-version definition rows.** `workflow_definitions` gains `source_rule_id uuid NULL REFERENCES room_booking_rules(id) ON DELETE SET NULL` + reuses existing `version integer` + `status` CHECK widened to include `'archived'`. Unique index on `(tenant_id, source_rule_id, version) WHERE source_rule_id IS NOT NULL`. Migration 00382.
4. **Threshold='any' semantics on the resolve path.** Migration 00382 adds `approvals.chain_threshold text NOT NULL DEFAULT 'all' CHECK (chain_threshold IN ('all','any'))`. Migration 00383 supersedes `grant_booking_approval` to honour the threshold — `'all'` keeps current semantics; `'any'` resolves on first approve + expires siblings; **all done under a per-booking row lock** that serialises double-resolve attempts (BLOCKER 2 closure). Backfill DERIVES `chain_threshold` from existing row shape (CRITICAL 5 closure).
5. **Backfill migration (00382)** producing one `workflow_definitions` row (`version=1`) per `room_booking_rules` row with a non-null `approval_config`. Definitions are `entity_type='booking'`, `status='published'`, `source_rule_id=rule.id`, owned by the rule's `tenant_id`, immutable-as-Phase-1-engine-requires.
6. **Atomic auto-recompile via PL/pgSQL RPC** (BLOCKER 1 closure). `ensure_room_booking_rule_workflow_definition(p_rule_id, p_tenant_id, p_graph_definition)` acquires a row lock on the rule, computes the next version under the lock, inserts the new definition, archives prior versions safe to archive, and flips the rule's FK — one transaction. Ships in 00382.
...
- **`:510-518`** — `if (approval.target_entity_type === 'booking')` → `grantBookingApproval` (RPC 00310 → v4 supersedes via 00383).
...
**Verified:** today the RPC fires `kind='resolved'` ONLY when (a) decision='rejected' OR (b) decision='approved' AND no siblings pending/rejected. **No path fires `kind='resolved'` on the first approve of a threshold='any' chain.** v4's 00383 (a) adds the 'any' branch under (b) a per-booking row lock (NOT advisory) acquired BEFORE the self-CAS. See §6.C.
...
**v4 ADDS to approvals:** `workflow_instance_id uuid`, `workflow_node_id text`, `chain_threshold text NOT NULL DEFAULT 'all' CHECK (chain_threshold IN ('all','any'))`. Migration 00382. Backfill DERIVES `chain_threshold` from existing row shape (CRITICAL 5 closure).
...
The RPC returns four kinds today: `non_booking_approved`, `already_responded`, `partial_approved`, `resolved`. **v4 extends the 00310 supersession (00383) to:** (a) honour `chain_threshold` for the `'any'` case, (b) serialise all resolution via a **per-booking ROW lock** acquired BEFORE the self-CAS (BLOCKER 2 closure), (c) re-observe sibling state under that lock to short-circuit the loser when a sibling already resolved the chain, (d) emit `outbox.emit('approval.granted', …)` on the `kind='resolved'` branch — and ONLY on that branch (no emit from the new `'already_resolved'` short-circuit path).
...
- Migration 00382 backfill — calls the RPC directly inside the migration's backfill block (the same RPC; one tx; no separate PL/pgSQL helper function needed — the RPC IS the helper). One-shot backfill loops over rules and calls the RPC per rule.
...
- **2 new PL/pgSQL RPCs (00382):** `ensure_room_booking_rule_workflow_definition` (atomic auto-recompile — BLOCKER 1) and `cancel_workflow_instance_with_approvals` (atomic cancel+expire — CRITICAL 4).
- **1 producer migration (00383):** supersede `grant_booking_approval` to honour `chain_threshold` under a per-booking row lock with sibling re-observation (BLOCKER 2) + emit `approval.granted` on `kind='resolved'`.
- **1 schema + backfill migration (00382):** the new columns + chain_threshold backfill via derive-algorithm + 3 tenant triggers + status CHECK widen + the two new RPCs above + per-rule call to `ensure_room_booking_rule_workflow_definition` for one-shot backfill.
...
**Preflight baseline (run before authoring 00382):**
...
00378, 00379, 00380, 00381 are TAKEN. 00375 is the only gap on disk — Phase 1.5 does NOT use 00375 (latest-prefix discipline). **Phase 1.5 owns 00382 + 00383.**

| # | File | Purpose |
|---|---|---|
| 00382 | `room_booking_rules_workflow_definition_fk.sql` | All schema additions (rule FK, approvals link columns, workflow_definitions lineage, chain_threshold) + status CHECK widen + 3 tenant triggers + unique index + 2 new RPCs (`ensure_room_booking_rule_workflow_definition`, `cancel_workflow_instance_with_approvals`) + chain_threshold backfill via the derive algorithm + per-rule one-shot backfill via `ensure_room_booking_rule_workflow_definition`. |
| 00383 | `grant_booking_approval_v2.sql` | Supersede 00310 to (a) acquire per-booking row lock before self-CAS, (b) re-observe sibling state under the lock, (c) honour `chain_threshold` for resolve, (d) emit `approval.granted` on `kind='resolved'` (not `'already_resolved'`). |

Split is intentional — 00382 is schema + the two new RPCs + backfill, 00383 is the supersession of the canonical grant RPC. A bug in 00383 doesn't roll back the backfill done in 00382.

### 3.2 Backfill SQL shape (00382, pseudocode)
...
- Set `chain_threshold = node.config.threshold` (validated against `('all','any')` at compile time + DB CHECK + 00383 defense-in-depth).
...
- **Single approver, threshold='all' vs 'any'.** Both compile to identical graph topology; the `chain_threshold` on inserted approval rows differs. 00383 honours the threshold and resolves correctly in both cases (single-approver-any = single-approver-all = resolve immediately on first response).
...
- 00382 is idempotent. **Strategy:** unique index `(tenant_id, source_rule_id, version)` + the backfill loop's `WHERE workflow_definition_id IS NULL` guard + the chain_threshold loop's `WHERE chain_threshold IS NULL` guard. Re-running 00382 on a partially-backfilled DB finds rules already linked → skips. Existing chains with chain_threshold populated → skipped.
- 00383 (RPC supersession) is `CREATE OR REPLACE FUNCTION` — already idempotent.
- The two new RPCs (`ensure_room_booking_rule_workflow_definition`, `cancel_workflow_instance_with_approvals`) are `CREATE OR REPLACE FUNCTION` in 00382 — also idempotent.
...
- **Threshold='any' resolve path** — 00383 honours `chain_threshold` UNDER a per-booking row lock. Sibling re-observation under the lock prevents double-resolve.
...
**Eight ordered sub-steps. 6.A.X (compiler) → 6.B (migration 00382) → 6.A (engine) → 6.A.Y (start overload) → 6.C (migration 00383 + error codes) → 6.D (handler) → 6.E (cutover + auto-recompile) → 6.G (cron backstop).** Each is a CHECKPOINT-2 implementation review (full-review + codex).
...
### 6.B — Schema + backfill + RPCs (migration 00382)
...
**Files:** `supabase/migrations/00382_room_booking_rules_workflow_definition_fk.sql`. ~5-7 commits (the migration is one file but iterations through review will be common; the two RPCs add bulk).
...
### 6.C — Producer: grant_booking_approval v2 (migration 00383)
...
**Files:** `supabase/migrations/00383_grant_booking_approval_v2.sql`, `apps/api/src/modules/approval/event-types.ts` (NEW), error-code registry (5 sites × 7 codes — though `chain.threshold_invalid` was already in v3's 5-code list; v4 adds 2 to the v3 list of 5 → 7 total). ~12-15 commits.
...
- **Idempotency.** Re-run 00382 → no new rows; backfill loop `WHERE workflow_definition_id IS NULL` skips; chain_threshold loop `WHERE chain_threshold IS NULL` skips.
...
- **Happy path (threshold='all'):** create rule with chain_threshold='all' + 2 approvers, create booking matching the rule → workflow starts → both grant → 00383 fires `kind='resolved'` → approval.granted emits → handler resumes → booking `confirmed`.
- **Happy path (threshold='any'):** rule + 3 approvers, threshold='any' → first approver grants → 00383's `'any'` branch fires `kind='resolved'` immediately → expires siblings → approval.granted emits → handler resumes → booking `confirmed`.
...
- `ls supabase/migrations/ | tail -15` — 00378–00381 confirmed taken. Phase 1.5 owns 00382 + 00383. (v3 owned 00381 + 00382 — bumped.)
...
- `supabase/migrations/00310_grant_booking_approval_rpc.sql:155-158` — per-booking advisory lock (the BLOCKER 2 site — v4's 00383 replaces with row lock).
...
- `supabase/migrations/00310_grant_booking_approval_rpc.sql:244-253` — `kind='resolved'` (v4 extends in 00383).
...
- `supabase/migrations/00382_room_booking_rules_workflow_definition_fk.sql` — FUTURE (schema + backfill + 3 triggers + 2 RPCs).
- `supabase/migrations/00383_grant_booking_approval_v2.sql` — FUTURE (RPC supersession for `chain_threshold` + booking row lock + `approval.granted` emit).
...
- **Slots owned:** 00382 (schema + 2 RPCs + backfill + 3 triggers), 00383 (grant_booking_approval v2 with booking row lock).
- **Smoke probe count:** 16 (minimum) — v3's 14 + concurrent-threshold='any' probe + start-path-archived-refusal probe.
- **Sub-steps:** 6.A.X (compiler — pure) → 6.B (migration 00382) → 6.A (engine) → 6.A.Y (start overload) → 6.C (migration 00383 + error codes) → 6.D (handler) → 6.E (cutover + auto-recompile RPC call) → 6.G (cron backstop). Sub-step 6.F (deleted in v3) stays deleted.
