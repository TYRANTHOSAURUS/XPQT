...
1. **`ApprovalConfigCompilerService` packaging.** Should the service live under `apps/api/src/modules/approval/` or `apps/api/src/modules/room-booking-rules/`? Recommendation: `apps/api/src/modules/approval/` — compiler is approval-domain logic; consumers import.
   ...
   > **RESOLVED 2026-05-16 (Q1):** Decided — the service lives at `apps/api/src/modules/approval/`. `ApprovalConfigCompilerService` is approval-domain logic and is packaged inside the approval module; the room-booking-rules service and other consumers import it from there. This is the decided home; the alternative (`apps/api/src/modules/room-booking-rules/`) is rejected because compiler ownership belongs with the approval domain, not the rule-authoring surface.
...
4. **Drift defense for Option C+ per-executor dispatch.** §2.4 + R1 specify a convention test. Pressure-test target: confirm AST walk OR grep test is feasible. Fallback: documentation convention + Phase 1.5 retro revisit.
   ...
   > **RESOLVED 2026-05-16 (Q4):** Formally DEFERRED to the workflow-editor hardening backlog. Rationale: the create/update graph-drift surface this question was protecting against was materially reduced by fix commit `25dfa0c8` (fail-closed RPC + a single TS `compileApprovalGraph` path used for BOTH the create and the update rule-workflow flows). Because create and update now run through one shared compilation path, they mint byte-identical `workflow_definitions.graph_definition` by construction — there is no longer a code path where create and update can structurally diverge. A standalone drift/convention test (AST walk or grep gate) is therefore a hardening nicety, not a correctness gap. It is deferred to the workflow-editor hardening backlog rather than blocking Phase 1.5.
...
5. **Service-rule sibling spec — schedule.** Compiler accepts `rule_type` discriminator. Pressure-test target: which workstream pulls the sibling spec. Recommendation: retire-the-column spec is a sibling spec AFTER both rule_types are migrated.
   ...
   > **RESOLVED 2026-05-16 (Q5):** Formally DEFERRED with no schedule commitment. The service-rule sibling spec is not scheduled within Phase 1.5 and carries no date or workstream commitment here. It is tracked in the booking-services roadmap (`docs/booking-services-roadmap.md`), not in this plan. Phase 1.5's compiler retains the `rule_type` discriminator so the sibling spec can slot in later without a re-architecture, but sequencing/ownership of that work is out of scope for this document.
...
6. **Auto-recompile interaction with admin-authored workflow_definitions (Phase 4).** Once Phase 4 admins can write workflow_definitions by hand (no `source_rule_id`), what happens when an admin assigns one to a rule via `workflow_definition_id`? Pressure-test target: should `ensure_room_booking_rule_workflow_definition` skip when the rule's current FK references a row with `source_rule_id IS NULL`? Recommendation: yes — bake the skip into the RPC body (`if exists (select 1 from workflow_definitions wd where wd.id = current_fk and wd.source_rule_id is null) then return current_fk as definition_id; …`). Phase 4 ships the explicit UX. **Defer the exact RPC body branch to 6.E impl time; document the decision then.**
   ...
   > **RESOLVED 2026-05-16 (Q6):** Formally DEFERRED to Phase 4. Current behavior (explicit, accepted for Phase 1.5): the recompile RPC (`ensure_room_booking_rule_workflow_definition`) ALWAYS mints a new published `workflow_definitions` version on rule create/update, even when the rule's current `workflow_definition_id` FK points at an admin-authored definition (`source_rule_id IS NULL`). The proposed "skip-when-current-FK-is-admin-authored" branch is NOT shipped in Phase 1.5. It is a Phase 4 deliverable, to be built alongside the admin workflow picker UX so the skip semantics and the explicit "this rule uses an admin-authored workflow" affordance ship together and stay consistent. This is a known, accepted limitation of Phase 1.5: until Phase 4, an auto-recompile on a rule whose FK was hand-pointed at an admin-authored definition will replace the FK with a freshly compiled version. No admin-authored definitions exist in Phase 1.5 (the Phase 4 authoring UX does not yet exist), so the limitation is latent, not active, during this phase.
...
8. **(NEW v4)** **Cron sweeper cadence.** `ApprovalCancelSweeperCron` is spec'd at every 5min. Pressure-test target: is 5min the right cadence? Phase 1.C's `WorkflowWaitSweeperCron` runs every 5min (verify at impl time). Recommendation: match Phase 1.C cadence; revisit if observability shows lag.
   ...
   > **RESOLVED 2026-05-16 (Q8):** LOCKED at 5 minutes. The `ApprovalCancelSweeperCron` cadence is fixed at every 5 minutes, matching Phase 1.C's `WorkflowWaitSweeperCron`. No further action — cadence is decided. (Future cadence changes, if observability ever shows lag, are an operational tuning concern, not an open plan question.)
...
9. **(NEW v4)** **`cancel_workflow_instance_with_approvals` RPC return shape — `claimed=false` vs error.** Currently the RPC returns `claimed=false, approvals_expired_ct=0` when it loses the race. TS treats this as a no-op + returns. Alternative: raise an exception so the TS caller can distinguish "lost race" from "no work to do". Recommendation: keep the return shape — the no-op semantics are correct (the workflow really was already cancelled by another worker); raising an exception would force TS to swallow it. Document the contract in the RPC's comment header.
   ...
   > **RESOLVED 2026-05-16 (Q9):** Decided — KEEP the current return-shape contract. The RPC returns `claimed=false, approvals_expired_ct=0` when it loses the claim race; it does NOT raise. This return shape IS the contract: callers branch on `claimed` (false ⇒ another worker already cancelled this instance, treat as a successful no-op and return; true ⇒ this worker performed the cancellation and `approvals_expired_ct` reports how many approval rows it expired). Raising an exception on lost-race was rejected because it would force the TS caller to catch-and-swallow a non-error condition, obscuring real failures. The return shape is documented in the RPC's comment header as the contract; callers MUST branch on `claimed` rather than treat a non-raising call as unconditional success.
