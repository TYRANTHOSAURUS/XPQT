-- Add 'reservation' kind to search_global. Filters via the
-- reservation_visibility_ids predicate added in 00157.
--
-- Replaces the function from 00151 (defense-in-depth) with the same body
-- plus a new reservations branch. All existing kinds and behaviors are
-- preserved verbatim.
...
begin
  ...
  -- Reservations -------------------------------------------------------------
  -- Hits filtered by reservation_visibility_ids (participant / rooms.read_all
  -- / rooms.admin). Search by space name, requester name, and ref number.
  if v_wants_all or 'reservation' = any(p_types) then
    return query
      ...
      select
        'reservation'::text,
        h.id,
        h.space_name as title,
        coalesce(h.requester_name, '—') || ' · ' || to_char(h.start_at, 'Mon DD, HH24:MI') as subtitle,
        h.breadcrumb,
        h.score,
        jsonb_build_object(
