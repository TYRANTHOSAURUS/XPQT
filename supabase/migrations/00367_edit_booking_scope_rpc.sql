-- B.4 Step 2F.1 — edit_booking_scope RPC.
--
-- Spec: docs/follow-ups/b4-booking-edit-pipeline.md §3.6.5 (decision table),
-- §7 step B.4.C (two-phase plan-then-commit recurrence cutover), §7.B.4.C
-- (chunk threshold). This RPC is the PG primitive backing the
-- `POST /reservations/:id/edit-scope` controller after the Step 2F TS
-- cutover lands (2F.3). 2F.1 ships migration + concurrency probes only —
-- no TS cutover, no controller changes.
--
-- ── Why a separate RPC (vs. looping edit_booking in TS) ─────────────────
--
-- Recurrence-scope edits are an atomic decision from the operator's POV
-- ("move this 52-week series to room B"). Per the CLAUDE.md architectural
-- rule, multi-table multi-row writes where partial state corrupts
-- invariants belong in ONE PL/pgSQL function. Looping `edit_booking` from
-- TS across N occurrences would:
--   1. Hold N separate command_operations idempotency rows instead of
--      one — replay semantics break if the operator retries mid-batch.
--   2. Defeat the "all-or-nothing" guarantee — a failure on occurrence 14
--      leaves 13 committed and 38 untouched, with no rollback path.
--   3. Multiply the advisory-lock-and-row-lock cost N times.
--
-- One RPC, one transaction, N rows. Either all writes commit or none do.
--
-- ── RPC contract ────────────────────────────────────────────────────────
--
-- p_plans :: jsonb array of { booking_id, plan } pairs. Each `plan` is the
--   same EditPlan shape that 00364 `edit_booking` consumes — booking,
--   slot_patches[], approval, asset_reservation_patches[], order_patches[],
--   work_order_sla_patches[], _resolution_at, etc.
--
-- TS plan-builder (lands in 2F.3) computes the in-scope booking_ids per
-- the operator's scope choice (`series` = every occurrence of the
-- recurrence_series_id; `this_and_following` = RecurrenceService.splitSeries
-- + the from-here ids), then assembles one EditPlan per occurrence and
-- passes the list to this RPC. The RPC trusts the caller's scope decision;
-- defense-in-depth: it verifies all booking_ids share the SAME
-- `recurrence_series_id` (non-NULL). Mixed series would mean TS smuggled
-- bookings from a different series; the RPC raises `mixed_series` 422.
--
-- p_dry_run = true → semantic + FK + approval-action computation per
-- occurrence, NO writes. Used by the 2F two-phase commit (B.4.C §7) to
-- aggregate all-or-nothing failures before the real call. Per-occurrence
-- outcomes accumulate in `per_occurrence[]`; blocking errors still RAISE.
--
-- p_dry_run = false → writes apply per occurrence; transaction-scoped so
-- any per-plan raise rolls back all writes across all occurrences.
--
-- ── Defense in depth (§7.B.4.C two-phase) ───────────────────────────────
--
-- Server-side cap: N > 200 raises `too_many_occurrences` 422. TS enforces
-- the operator-confirmation threshold at 100 (per §7.B.4.C); this RPC's
-- 200 is a hard ceiling for the case where TS regresses or a non-HTTP
-- caller bypasses the confirmation.
--
-- ── B.4.A.5 controller-vs-notification gate (third emit site) ───────────
--
-- editOne (00364) + editSlot + this RPC are the three sites that can emit
-- `booking.approval_required` on §3.6.5 rows 2/7/8. Until B.4.A.5 ships
-- notification dispatch, every emit site MUST refuse plans that would
-- emit. TS preflight rejects at the controller layer; this RPC defends
-- against any TS regression or non-HTTP caller. The predicate matches
-- editSlot/editOne exactly:
--   plan.approval.new_outcome = 'require_approval' AND
--     (plan.approval.old_outcome != 'require_approval' OR
--      plan.approval.chain_config_changed = true)
-- → raise `booking.edit_requires_notification_dispatch` 422 with the
-- offending booking_id.
--
-- ── Approval reconciliation per occurrence ──────────────────────────────
--
-- Each occurrence is reconciled INDEPENDENTLY per the §3.6.5 10-row
-- decision table — same logic as 00364:456-580. Reconciliation is by
-- occurrence: scope=series with mixed approval states (some pending, some
-- terminal_approved) is legitimate and each row is treated on its own.
-- The RPC short-circuits on the first blocking row (row 5/9 cancelled,
-- row 10 deny) — TS sees the offending booking_id in the error message
-- and can surface "occurrence N blocked the whole batch".
--
-- ── Citation discipline ────────────────────────────────────────────────
--
-- - Bookings columns (recurrence_series_id, recurrence_index,
--   recurrence_overridden): 00277:74-76.
-- - Recurrence series table: 00124:5-17.
-- - command_operations idempotency contract: 00316:32-42.
-- - Approval expire shape: 00310:165-172 (mirrored in 00364:879-904).
-- - Approval INSERT shape: booking-flow.service.ts:1275-1283 (mirrored in
--   00364:906-927).
-- - validate_entity_in_tenant kinds (person/team/space/cost_center/
--   booking_rule/work_order/asset): 00360:173-235.
-- - Outbox emit signature: outbox.emit(p_tenant_id, p_event_type,
--   p_aggregate_type, p_aggregate_id, p_payload, p_idempotency_key,
--   p_event_version, p_available_at) — same shape as 00364:1020-1100.
-- - hashtextextended for advisory lock: 00364:374 (same pattern).
--
-- ── Raise codes (new) ──────────────────────────────────────────────────
...
--
-- ── Raise codes (reused — already registered) ──────────────────────────
...
begin
  ...
  v_payload_hash := md5(coalesce(p_plans::text, '') || ':' || coalesce(p_dry_run::text, ''));
  ...
  -- One round-trip; checks tenant + existence + cross-tenant defense.
  -- Sort by id ASC matches array order so the rows we visit later via
  -- jsonb_array_elements(p_plans) line up to the same set. Per-plan
  -- lookups inside the loop re-read by id (idempotent under the lock).
  ...
  -- Verify every booking_id was present.
  if (select count(*) from public.bookings
       where id = any(v_booking_id_set) and tenant_id = p_tenant_id)
     <> array_length(v_booking_id_set, 1) then
    raise exception 'edit_booking_scope.booking_not_found: one or more booking_ids not found in tenant=% (requested=%)',
      p_tenant_id, v_booking_id_set
      using errcode = 'P0001';
  end if;

  -- ── 6. Defense-in-depth: same-series check ──────────────────────────
  -- Every booking_id must share the SAME non-NULL recurrence_series_id.
  -- NULL series_id means the booking isn't part of a recurrence — illegal
  -- input for this RPC. Mismatched series_ids means TS smuggled a booking
  ...
  --
  -- For each {booking_id, plan}:
  --   (a) lock + reload the booking row (already locked above; this just
  --       reads the snapshot for diff/audit).
  --   (b) cancelled-state guard.
  --   (c) semantic re-derivation gate (room_booking_rules updated_at).
  --   (d) tenant-validate all FKs in the plan.
  --   (e) approval reconciliation per §3.6.5.
  --   (f) B.4.A.5 controller-vs-notification gate (defense-in-depth).
  --   (g) If p_dry_run=true: record per_occurrence outcome, continue.
  --   (h) If p_dry_run=false: apply writes (booking_slots, bookings,
  --       asset_reservations, orders, approvals, work_orders) +
  --       audit_events + domain_events + outbox emits.
  --
  -- Loop is ORDER-PRESERVING over p_plans for predictable per_occurrence
  -- ordering in the response. Sort independence is guaranteed by the
  ...
  for v_plan_elem in select * from jsonb_array_elements(p_plans) loop
    ...
    -- REQUIRED booking-patch keys (mirrors 00364:339-355).
    ...
    -- Decision-table resolution (mirrors 00364:482-558). Action +
    -- status_target derived from (old, new, state, chain_changed).
    ...
    -- Terminal raises before child validation (mirrors 00364:561-572).
    ...
    -- Predicate matches editSlot/editOne. The TS preflight in Step 2F.3
    -- catches this earlier; defense-in-depth here for non-HTTP callers.
    ...
    -- 00364:577-633). Reachable only if v_emit_approval_required would
    -- have stayed false but action is still insert/expire_and_insert
    -- (no such path post B.4.A.5 gate; defensive carryover from 00364).
    ...
    -- 7.g — tenant-validate every other FK in the plan (mirrors
    -- 00364:635-733). Order: space (booking + per-slot), cost_center,
    -- host_person, applied_rule_ids, asset, work_order (+ booking-scope
    -- check), order (+ booking-scope check), asset_reservation
    -- (+ booking-scope check).
    ...
    -- 7.h — derive new booking values (mirrors 00364:735-788).
    ...
    if p_dry_run then
      -- follow_ups_preview: emits that WOULD fire on commit. Note: with
      -- the B.4.A.5 gate at 7.e, approval_required can never actually
      -- emit; it's preserved here for symmetry with the post-cutover
      -- shape.
    ...
    -- 7.j — atomic write block (mirrors 00364:790-944).
    for v_slot in select * from jsonb_array_elements(v_slot_patches) loop
    ...
    -- 7.k — audit + domain_events per occurrence (mirrors 00364:946-1016).
    ...
    -- 7.l — outbox emits per occurrence (mirrors 00364:1018-1103).
    if v_booking.location_id is distinct from v_new_location_id then
      perform outbox.emit(
    ...
    -- booking.approval_required is NEVER emitted from this RPC — the
    -- B.4.A.5 gate at step 7.e refuses any plan that would have set
    -- v_emit_approval_required=true. Listed in the spec for completeness.
    ...
    -- 7.m — accumulate per-occurrence outcome.
    v_per_occurrence := v_per_occurrence || jsonb_build_object(
      'booking_id',      v_target_booking_id,
      'slots_updated',   v_slots_updated,
      'assets_updated',  v_assets_updated,
...
comment on function public.edit_booking_scope(jsonb, uuid, uuid, text, boolean) is
  'B.4 Step 2F.1 — edit_booking_scope RPC. Atomic write across N occurrences of a recurrence series. Mirrors edit_booking (00364) per-occurrence semantics — semantic re-derivation gate, approval reconciliation per §3.6.5, FK validation via validate_entity_in_tenant (v5 kinds: person/team/space/cost_center/booking_rule/work_order/asset). Defense-in-depth: (1) all booking_ids must share the same non-null recurrence_series_id (raises edit_booking_scope.mixed_series 422 on mismatch); (2) N > 200 hard cap (raises edit_booking_scope.too_many_occurrences 422 — TS-side confirmation threshold lives at 100 per §7.B.4.C); (3) B.4.A.5 controller-vs-notification gate at every approval-required emit site refuses plans before notification dispatch ships. p_dry_run=true skips writes + accumulates per-occurrence outcomes for the two-phase plan-then-commit pattern. Spec: docs/follow-ups/b4-booking-edit-pipeline.md §3.6.5 + §7.B.4.C.';
