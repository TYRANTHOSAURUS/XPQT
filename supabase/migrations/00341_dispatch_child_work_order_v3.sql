-- B.2.A.Step8 codex remediation — dispatch_child_work_order RPC (§3.4 single) — v3.
--
-- Spec:        docs/follow-ups/b2-survey-and-design.md §3.4.
-- Replaces:    00338 (v2).
-- Companion:   00342 (batch v3).
-- Helper:      00340 (validate_entity_in_tenant v3 — adds 'routing_rule').
--
-- ── What this revision fixes ────────────────────────────────────────────
--
-- **F-IMP-1 (codex-S8-I1) — routing_rule_id tenant validation.**
-- v2 (00338:163) parsed `routing_rule_id` from the payload and inserted
-- it into `routing_decisions.rule_id` (00338:317) WITHOUT validating
-- tenant ownership. The FK on `routing_decisions.rule_id` is GLOBAL
-- (00027:67) — no tenant_id composite — so a forged or internal-bug
-- payload could write tenant A's audit row pointing at tenant B's rule.
-- Cross-tenant leak in `routing_decisions.trace` audit data.
--
-- Fix: call `validate_entity_in_tenant(p_tenant_id, 'routing_rule',
-- v_routing_rule_id)` (extended in 00340) after the other top-level FK
-- validations + before the `routing_decisions` INSERT. Matches the
-- defense-in-depth pattern for asset / location / request_type / sla.
--
-- Mirrors 00338 in every other respect. Schema-cache reload at the
-- bottom so PostgREST picks up the new body.
