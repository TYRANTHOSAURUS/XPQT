-- Defence-in-depth for the global search RPC --------------------------------
--
-- The grant lockdown in 00137 already prevents authenticated/anon callers
-- from invoking `search_global` directly via PostgREST, so cross-tenant
-- traffic must currently go through `/api/search` where Nest resolves
-- (auth_uid → user.id) within the caller's tenant.
--
-- This migration adds an internal assert so the function refuses to run
-- if `(p_user_id, p_tenant_id)` doesn't reference a real user in that
-- tenant. That makes the contract self-enforcing — even if a future
-- internal caller (an MCP server, a backfill script, a different service)
-- bypasses the API layer, search_global will not return rows for a
-- mismatched (user, tenant) pair.
...
begin
  ...
  -- Defence in depth: refuse if the (user, tenant) pair isn't real, even
  -- though the grant lockdown already prevents direct PostgREST calls.
