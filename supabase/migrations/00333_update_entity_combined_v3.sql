-- B.2.A.8 — update_entity_combined v3 (§3.0 orchestrator).
--
-- Spec:        docs/follow-ups/b2-survey-and-design.md §3.0 (lines 1781-1897).
-- Supersedes:  00332 (same function signature; CREATE OR REPLACE).
-- Predecessor: 00331 (v1) → 00332 (v2, F1-F4) → 00333 (v3, F8-F10).
--
-- ── Why v3 (codex remediation on 2b4a31a) ────────────────────────────
--
-- Three codex-surfaced findings folded in:
--
-- F8. metadata.tags / metadata.watchers null clears (codex F8). v2 at
--     00332:557-562 and 00332:582-586 rejected a jsonb-null value for
--     metadata.tags / metadata.watchers with `invalid_metadata`. The
--     TS surface treats null as the explicit-clear sentinel — see
--     work-order.service.ts:1483-1487 (DTO permits `tags?: string[] |
--     null` + `watchers?: string[] | null`) and the no-op diff at
--     work-order.service.ts:1596-1609 which writes
--     `next = dto.tags ?? null`. The DB columns are NULLABLE (default
--     '{}' but not NOT NULL) per 00011_tickets.sql:25-27 and
--     00213_step1c1_work_orders_new_table.sql:80-82. v3 accepts
--     jsonb-null on both fields and persists SQL null — matching the
--     TS pathway so the orchestrator and the direct TS update routes
--     agree on the clear semantics.
--
-- F9. Watcher validation parity with TS contract (codex F9). v2 at
--     00332:609-614 ran `select count(*) from persons where
--     tenant_id = ... and id = any(...)` — passing any tenant row, even
--     a deactivated / GDPR-anonymized / off-boarded one. The canonical
--     TS helper at tenant-validation.ts:271-302 enforces:
--       • size cap of MAX_WATCHER_IDS_PER_QUERY (= 200) UNIQUE uuids
--         (tenant-validation.ts:235 + :271-273);
--       • the filter `active = true AND anonymized_at IS NULL AND
--         left_at IS NULL` (tenant-validation.ts:295-302).
--     v3 mirrors both: raise `update_entity_combined.invalid_watcher`
--     when the unique set exceeds 200; tighten the persons lookup so
--     stale persons (deactivated / anonymized / departed) cannot be
--     added as watchers via the orchestrator. Existing watchers are
--     not touched at write-time, matching the TS comment at
--     tenant-validation.ts:289-294. Columns verified: `active` exists
--     at 00003_people_users_roles.sql:17; `anonymized_at` + `left_at`
--     exist at 00164_gdpr_persons_visitors_anonymization.sql:14-19.
--
-- F10. Preserve watcher ARRAY ORDER through dedup (codex F10). v2 at
--     00332:607 used `array(select distinct unnest(v_new_watchers))`.
--     `distinct` does not commit to any element ordering — the result
--     is implementation-defined and frequently appears as set-hash
--     order. The downstream comparison at 00332:621
--     (`v_new_watchers is distinct from v_prev_watchers`) is order-
--     sensitive for text[]/uuid[], so an input like `[B,A]` could be
--     persisted as `[A,B]` and then false-flag as changed on every
--     replay even though the deduped set is identical. v3 dedupes via
--     `DISTINCT ON (elem)` over an `UNNEST … WITH ORDINALITY` so each
--     unique uuid keeps its FIRST occurrence's position; the caller's
--     intended order survives the dedup. Reference: PG docs on
--     `WITH ORDINALITY` (set-returning function add-on returning a
--     bigint position column) + `DISTINCT ON (expr)` (one row per
--     `expr` value, keeping the row that sorts first under the outer
--     ORDER BY).
--
-- All other v2 behaviour (F1 inner-key sentinel, F2 dedup-before-
-- tenant-count, F3 jsonb_typeof checks before cast → registered
-- codes, F4 hoisted v_actor_person_id resolution; argument shape
  ...
  if v_metadata_branch then
    ...
    if v_has_watchers_key then
      ...
      if jsonb_typeof(v_watchers_raw) = 'null' then
        ...
        -- the outer ORDER BY ord re-applies original order. Without
        -- WITH ORDINALITY, `select distinct unnest(...)` returns
        -- unspecified order, which makes the `is distinct from`
        -- comparison below (still array-order-sensitive) misbehave on
        -- inputs like `[B, A]`.
...
comment on function public.update_entity_combined(text, uuid, uuid, uuid, text, jsonb) is
  'B.2.A.8 §3.0 orchestrator v3 (00333 supersedes 00332). Same surface; folds F1 (inner-key sentinel __combined__:), F2 (watcher dedup before tenant check + persist deduped), F3 (jsonb_typeof validation BEFORE cast → registered codes), F4 (one actor_person_id lookup per call), F8 (metadata.tags/watchers null is explicit-clear, mirrors work-order.service.ts:1483-1487 + :1596-1609), F9 (watcher validation parity with tenant-validation.ts:271-302 — 200 unique-uuid cap + active=true + anonymized_at IS NULL + left_at IS NULL filter; persons.active at 00003:17, persons.anonymized_at/left_at at 00164:14-19), F10 (dedup preserves caller insertion order via DISTINCT ON over UNNEST WITH ORDINALITY). Composes per-field RPCs (transition_entity_status 00325 / set_entity_assignment 00327 / update_entity_sla 00330) plus inline plan / priority / metadata branches mirroring ticket.service.ts (priority :1224-1234; metadata :1236-1252) + work-order.service.ts (plan :942-968). Plan-on-case rejected (00011_tickets.sql:1-44 has no planned_* cols). Nested idempotency keys are sentinel-prefixed; outer cache hit short-circuits before re-entering sub-RPCs. Atomicity: every branch in one tx; one raise rolls everything back. Watchers persisted as the deduped, order-preserved set. Cost rounded to 2dp for numeric(12,2) round-trip parity. Result: {entity_id, entity_kind, branches_applied, status, assignment, sla, priority, plan, metadata, any_changed, noop}. Idempotent on (tenant_id, p_idempotency_key) via command_operations (00316). Spec: docs/follow-ups/b2-survey-and-design.md §3.0.';
