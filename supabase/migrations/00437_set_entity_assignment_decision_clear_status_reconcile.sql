-- Audit 02 reconcile: set_entity_assignment merged v3 body.
--
-- Supersedes:
--   * 00419_set_entity_assignment_v3_2_chosen_provenance_guard.sql
--     (watchers + decision provenance + chosen_* guard)
--   * 00433_restore_set_entity_assignment_main_v3_after_remote_reconcile.sql
--     (case-only clear_routing_status guard + no timeline/domain noise for
--      pure routing-status clears)
--
-- 00433 restored the main-line clear_routing_status body but dropped the
-- decision payload support used by TicketService.reassign(rerun_resolver).
-- The merged API sends p_payload.decision so routing_decisions carries
-- resolver provenance instead of hardcoded manual/manual_reassign. This
-- migration reconciles both branches forward in one canonical body.
...
begin
  ...
  -- 00433 guard: work_orders do not have routing_status /
  -- routing_failure_reason columns. Reject the unsupported directive before
  -- writing command_operations so a bad producer leaves no idempotency row.
  ...
  -- ── 14+15 gate (00433 reconcile) ───────────────────────────────────────
  -- clear_routing_status and decision can force the write path even when
  -- assignees/watchers are unchanged. In that case the meaningful writes are
  -- the case routing-status reset and routing_decisions provenance row; do
  -- not emit blank ticket_activities/domain_events.
  if v_team_changed or v_user_changed or v_vendor_changed or v_watchers_changed or v_reason is not null then
...
comment on function public.set_entity_assignment(uuid, text, uuid, uuid, text, jsonb) is
  'Atomic assignment change for cases (tickets) and work_orders. Reconciled v3 body: watcher replacement, decision-based routing_decisions provenance with chosen_* guards, and case-only clear_routing_status. Pure routing-status clears write the case status reset and decision audit row without blank ticket_activities/domain_events; work_order + clear_routing_status raises set_entity_assignment.routing_status_unsupported_for_work_order. Spec: docs/follow-ups/audits/02-tickets-work-orders.md.';
