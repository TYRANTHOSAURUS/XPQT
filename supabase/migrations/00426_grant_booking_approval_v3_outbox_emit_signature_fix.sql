-- Phase 1.5 — Visual approval workflow — sub-step 6.C (v3 hotfix).
--
-- BUG FIX (smoke gate finding, smoke-visual-approval probes 1-4/6):
--   00403_grant_booking_approval_v2.sql:364-374 calls the outbox producer
--   as a 2-arg `outbox.emit('approval.granted', jsonb_build_object(...))`.
--   The ONLY outbox.emit signature that exists anywhere in the migration
--   tree is the 8-arg canonical one defined in 00299_outbox_foundation.sql
--   :132 — `outbox.emit(p_tenant_id uuid, p_event_type text,
--   p_aggregate_type text, p_aggregate_id uuid, p_payload jsonb,
--   p_idempotency_key text, p_event_version int, p_available_at
--   timestamptz)`. No 2-arg `outbox.emit(text, jsonb)` overload was ever
--   authored. Result: every Phase 1.5 visual-approval grant where
--   `workflow_instance_id IS NOT NULL` (the entire happy path) raised
--   `function outbox.emit(unknown, jsonb) does not exist` and 500'd —
--   the workflow never advanced past the grant. Mocked-jest + code review
--   could not catch this; the live-API smoke gate did (CLAUDE.md "Smoke
--   gates" rationale).
--
--   This migration is byte-identical to 00403 EXCEPT the outbox.emit call
--   now uses the canonical 8-arg named-argument form, mirroring the
--   established pattern in 00373_delete_booking_emit_cancelled.sql:195-209
--   and every other emit call site in the tree. Idempotency key is
--   `approval.granted:<approval_id>` — stable per approval resolution so
--   concurrent / retried emits (BLOCKER 2 race, outbox redelivery)
--   converge to one row via outbox.emit's (tenant_id, idempotency_key)
--   ON CONFLICT dedup (00299:161-192); resume()'s atomic claim remains
--   the in-engine idempotency guarantee.
--
-- Supersedes `public.grant_booking_approval` (00310/00403) with
-- chain_threshold-aware resolve semantics + a per-booking ROW lock at the
-- top of the body (BLOCKER 2 closure) + an outbox.emit('approval.granted')
-- signal on kind='resolved' so WorkflowApprovalGrantedHandler (sub-step
-- 6.D) can resume() the parent workflow_instance.
--
...
begin
  ...
  if v_workflow_instance_id is not null then
    -- Canonical 8-arg outbox.emit (00299:132). Named args, mirroring
    -- 00373:195-209. Idempotency key is stable per approval resolution:
    -- concurrent / redelivered emits of the same resolved approval (BLOCKER
    -- 2 race, outbox retry) collapse to one row via the (tenant_id,
    -- idempotency_key) ON CONFLICT dedup (00299:161-192). The payload is
    -- built from immutable resolution columns so same-key re-emits carry an
    -- identical payload_hash → silent idempotent success (not a 23505).
    perform outbox.emit(
      p_tenant_id       => p_tenant_id,
      p_event_type      => 'approval.granted',
      ...
      p_idempotency_key => 'approval.granted:' || p_approval_id::text,
      p_event_version   => 1,
      p_available_at    => null
    );
  end if;
