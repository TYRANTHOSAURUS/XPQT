-- B.2.A.Step8 — dispatch_child_work_orders_batch RPC (§3.4 batch) — v2.
--
-- Spec:        docs/follow-ups/b2-survey-and-design.md §3.4 (lines 2228-2234).
-- Replaces:    00337 (v1).
-- Companion:   00338 (single v2).
--
-- ── What this revision fixes ────────────────────────────────────────────
--
-- 1. **Polymorphic sla_timers columns set on insert** (F-CRIT-1). Same
--    fix as 00338: every sla_timers row inserted by the batch path now
--    sets (entity_kind='work_order', case_id=null, work_order_id=v_child_id,
--    started_at=now()) plus (paused=false, recompute_pending=false).
--    Mirrors 00330:259-277 exactly.
--
-- 2. **Unreachable coalesce arm dropped** (F-IMP-7). priority coalesce
--    drops the 'medium' fallback — tickets.priority is NOT NULL DEFAULT
--    'medium' (00011:14) so v_parent.priority is never null.
--
-- 3. **Per-task validate_assignees_in_tenant** (F-IMP-3). Verified
--    already present in v1's per-task loop (00337:242-247). No change
--    needed; the harness scenario 2 (cross-tenant assignee in task #2)
--    already exercises the per-task path.
--
-- 4. **routing_trace embedded ids — audit-only**. Same documented
--    limitation as the single RPC (see 00338 header).
...
comment on function public.dispatch_child_work_orders_batch(uuid, uuid, uuid, text, jsonb) is
  'v2 (00339) — Atomic batch dispatch of N child work_orders from a parent case. Fixes F-CRIT-1 (sla_timers polymorphic columns now populated per-task; mirrors 00330:259-277) + F-IMP-7 (drops unreachable coalesce arm). Per-task validate_assignees_in_tenant inside the loop preserves all-or-nothing semantics. Spec: docs/follow-ups/b2-survey-and-design.md §3.4.';
