-- 00416_floor_plan_drafts_and_set_entity_assignment_v3.sql
-- BUNDLED migration: combines the two files that previously both
-- claimed version 00416. Supabase tracks migrations by leading version
-- prefix, so a duplicate violated schema_migrations_pkey on CI db:reset.
--
-- Renumbering wasn't viable: 00417_floor_plans_and_drafts_labels.sql
-- depends on `public.floor_plan_drafts` existing at <= 00417, so the
-- floor-plan file couldn't move forward; and several later migrations
-- (00418/00419/00425/00433/00437) extend `public.set_entity_assignment`
-- and would break if its base v3 creation moved past them. Bundling
-- preserves both downstream chains in one file at the original prefix.
--
-- Remote prod has both contents applied already (pushed via direct
-- psql, no schema_migrations tracking); idempotency guards in Section 1
-- make local-stack re-application safe. Section 2 is CREATE OR REPLACE
-- FUNCTION which is naturally idempotent.
--
-- ── Section 1: floor_plan_drafts (originally 00416_floor_plan_drafts.sql)
-- Per-floor in-progress edits. One draft per floor. Polygons stored as
-- jsonb so the booking surface keeps reading the published
-- spaces.floor_plan_polygon without ever seeing half-edited state.
-- Spec §3.3.
