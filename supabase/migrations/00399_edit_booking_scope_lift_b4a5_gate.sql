-- 00399 — edit_booking_scope: lift B.4.A.5 controller-vs-notification gate
--
-- Sub-step H of B.4.A.5 (notification dispatch). Drops the in-PG 422
-- raise that 00395 carried at lines 562-568 ("booking.edit_requires_
-- notification_dispatch"). Companion to the TS-side gate lifts at
-- reservation.service.ts (editOne + editSlot) and
-- assemble-edit-plan.service.ts (editScope per-occurrence loop) shipped
-- in the same commit. Approval-flipping scope-mode edits now succeed at
-- the DB layer, write inbox rows atomically, and emit outbox events for
-- email dispatch — same Hybrid-C path that editOne/editSlot already use.
--
-- The 'booking.edit_requires_notification_dispatch' error code stays
-- registered (packages/shared/src/error-codes.ts + EN/NL messages) for
-- defense-in-depth — any future regression that re-introduces the gate
-- must reuse this code.
--
-- Signature: unchanged from 00395.
-- Pattern: create or replace function — no drop, no cascade.
