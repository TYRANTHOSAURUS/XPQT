-- B.2.A.8 — update_entity_combined v5 (§3.0 orchestrator).
--
-- Spec:        docs/follow-ups/b2-survey-and-design.md §3.0 (lines 1781-1897).
-- Supersedes:  00334 (same function signature; CREATE OR REPLACE).
-- Predecessor: 00331 (v1) → 00332 (v2) → 00333 (v3) → 00334 (v4) → 00335 (v5).
--
-- ── Why v5 (codex CODEX-B-2 — gate post-SLA hook on actual timer insertion) ─
--
-- v4 (00334:744-766) added a post-SLA recompute hook: when the sla branch
-- runs AND the entity ends in `status_category='waiting'`, the orchestrator
-- bumps `recompute_pending=true` on the fresh active timers + emits one
-- additional outbox 'sla.timer_recompute_required' event. The condition
-- gate was `v_branches_applied @> '["sla"]'::jsonb` — anywhere sla was
-- present.
--
-- But the sla sub-RPC has a no-op fast path (00330:179-194): when the
-- payload sla_id matches the current sla_id AND no timers were sent, the
-- sub-RPC returns `{..., timers_inserted: 0, noop: true}` without writing
-- any rows (no STOP, no INSERT, no UPDATE on entity row's sla columns).
-- Under v4, the orchestrator's post-sla hook STILL fired in that case —
-- bumping recompute_pending=true on whatever timers existed (potentially
-- ones that the status branch had just paused) AND emitting a redundant
-- outbox event with action='post_sla_install_in_waiting' that doesn't
-- describe reality (no fresh timers were installed).
--
-- v5 fix: gate the hook on `coalesce((v_sla_result->>'timers_inserted')::int, 0) > 0`
-- IN ADDITION to the branch-applied check. The sub-RPC's contract returns
-- `timers_inserted` as the row count from the fresh INSERT
-- (00330:286: `get diagnostics v_inserted_count = row_count`). When 0,
-- nothing was installed and the post-sla hook has nothing to recompute.
--
-- All other v4 behaviour (F1 inner-key sentinel, F2 dedup-before-tenant-
...
-- atomicity across branches, sub-RPC result-shape contract, v4 post-SLA
-- recompute hook structure) is preserved verbatim.
...
begin
  ...
  -- If the sla branch inserted fresh timers AND the entity is now in
  -- 'waiting' state (either from a concurrent status branch in this
  -- call or from prior state), bump recompute_pending=true on the
  -- fresh timers so the SlaTimerHandler worker re-evaluates pause
  -- state under the new policy.
  --
  -- v5 / codex CODEX-B-2: gate hook on actual timer insertion. The
  -- sla sub-RPC's no-op fast path (00330:179-194) returns
  -- `timers_inserted=0, noop=true` without writing rows; the post-sla
  -- recompute hook must not fire in that case (avoid spurious outbox
  -- emit + redundant UPDATE that would touch unrelated active timers).
  -- The sub-RPC's contract returns `timers_inserted` as the row count
  -- from the fresh INSERT at 00330:286 (`get diagnostics
  -- v_inserted_count = row_count`). When 0, no install happened.
  --
